...
public partial class JUN26 {
    ...
    /// appointmentCldrList
    /// </summary>
    public static AppointmentCldrList appointmentCldrList
    {
        get => HttpData.Get<AppointmentCldrList>("appointmentCldrList")!;
        set => HttpData["appointmentCldrList"] = value;
    }

    /// <summary>
    /// Page class for Appointment_Cldr
    /// </summary>
    public class AppointmentCldrList : AppointmentCldrListBase
    {
        // Constructor
        public AppointmentCldrList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public AppointmentCldrList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class AppointmentCldrListBase : AppointmentCldr
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "appointmentCldrList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fAppointment_Cldrlist";
        ...
        // Constructor
        public AppointmentCldrListBase()
        {
            ...
            MultiDeleteUrl = "AppointmentCldrDelete";
            MultiUpdateUrl = "AppointmentCldrUpdate";
        ...
        // Page name
        public string PageName => "AppointmentCldrList";
        ...
        // Constructor
        public AppointmentCldrListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fAppointment_Cldrgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                appointmentCldrList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fAppointment_Cldrsrch");
            ...
            filters.Merge(JObject.Parse(_Title.AdvancedSearch.ToJson())); // Field Title
            filters.Merge(JObject.Parse(Start.AdvancedSearch.ToJson())); // Field Start
            filters.Merge(JObject.Parse(End.AdvancedSearch.ToJson())); // Field End
            filters.Merge(JObject.Parse(AllDay.AdvancedSearch.ToJson())); // Field AllDay
            filters.Merge(JObject.Parse(Description.AdvancedSearch.ToJson())); // Field Description
            filters.Merge(JObject.Parse(_Url.AdvancedSearch.ToJson())); // Field Url
            filters.Merge(JObject.Parse(Display.AdvancedSearch.ToJson())); // Field Display
            filters.Merge(JObject.Parse(BackgroundColor.AdvancedSearch.ToJson())); // Field BackgroundColor
            filters.Merge(JObject.Parse(CRSS_ID.AdvancedSearch.ToJson())); // Field CRSS_ID
            filters.Merge(JObject.Parse(str_AppointmentType.AdvancedSearch.ToJson())); // Field str_AppointmentType
            filters.Merge(JObject.Parse(str_AppointmentStatus.AdvancedSearch.ToJson())); // Field str_AppointmentStatus
            filters.Merge(JObject.Parse(str_Username.AdvancedSearch.ToJson())); // Field str_Username
            filters.Merge(JObject.Parse(str_CRSS_IDUSER.AdvancedSearch.ToJson())); // Field str_CRSS_IDUSER
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fAppointment_Cldrsrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Title
            if (filter?.TryGetValue("x__Title", out sv) ?? false) {
                _Title.AdvancedSearch.SearchValue = sv;
                _Title.AdvancedSearch.SearchOperator = filter["z__Title"];
                _Title.AdvancedSearch.SearchCondition = filter["v__Title"];
                _Title.AdvancedSearch.SearchValue2 = filter["y__Title"];
                _Title.AdvancedSearch.SearchOperator2 = filter["w__Title"];
                _Title.AdvancedSearch.Save();
            }

            // Field Start
            if (filter?.TryGetValue("x_Start", out sv) ?? false) {
                Start.AdvancedSearch.SearchValue = sv;
                Start.AdvancedSearch.SearchOperator = filter["z_Start"];
                Start.AdvancedSearch.SearchCondition = filter["v_Start"];
                Start.AdvancedSearch.SearchValue2 = filter["y_Start"];
                Start.AdvancedSearch.SearchOperator2 = filter["w_Start"];
                Start.AdvancedSearch.Save();
            }

            // Field End
            if (filter?.TryGetValue("x_End", out sv) ?? false) {
                End.AdvancedSearch.SearchValue = sv;
                End.AdvancedSearch.SearchOperator = filter["z_End"];
                End.AdvancedSearch.SearchCondition = filter["v_End"];
                End.AdvancedSearch.SearchValue2 = filter["y_End"];
                End.AdvancedSearch.SearchOperator2 = filter["w_End"];
                End.AdvancedSearch.Save();
            }

            // Field AllDay
            if (filter?.TryGetValue("x_AllDay", out sv) ?? false) {
                AllDay.AdvancedSearch.SearchValue = sv;
                AllDay.AdvancedSearch.SearchOperator = filter["z_AllDay"];
                AllDay.AdvancedSearch.SearchCondition = filter["v_AllDay"];
                AllDay.AdvancedSearch.SearchValue2 = filter["y_AllDay"];
                AllDay.AdvancedSearch.SearchOperator2 = filter["w_AllDay"];
                AllDay.AdvancedSearch.Save();
            }

            // Field Description
            if (filter?.TryGetValue("x_Description", out sv) ?? false) {
            ...
            // Field Url
            if (filter?.TryGetValue("x__Url", out sv) ?? false) {
                _Url.AdvancedSearch.SearchValue = sv;
                _Url.AdvancedSearch.SearchOperator = filter["z__Url"];
                _Url.AdvancedSearch.SearchCondition = filter["v__Url"];
                _Url.AdvancedSearch.SearchValue2 = filter["y__Url"];
                _Url.AdvancedSearch.SearchOperator2 = filter["w__Url"];
                _Url.AdvancedSearch.Save();
            }

            // Field Display
            if (filter?.TryGetValue("x_Display", out sv) ?? false) {
                Display.AdvancedSearch.SearchValue = sv;
                Display.AdvancedSearch.SearchOperator = filter["z_Display"];
                Display.AdvancedSearch.SearchCondition = filter["v_Display"];
                Display.AdvancedSearch.SearchValue2 = filter["y_Display"];
                Display.AdvancedSearch.SearchOperator2 = filter["w_Display"];
                Display.AdvancedSearch.Save();
            }

            // Field BackgroundColor
            if (filter?.TryGetValue("x_BackgroundColor", out sv) ?? false) {
                BackgroundColor.AdvancedSearch.SearchValue = sv;
                BackgroundColor.AdvancedSearch.SearchOperator = filter["z_BackgroundColor"];
                BackgroundColor.AdvancedSearch.SearchCondition = filter["v_BackgroundColor"];
                BackgroundColor.AdvancedSearch.SearchValue2 = filter["y_BackgroundColor"];
                BackgroundColor.AdvancedSearch.SearchOperator2 = filter["w_BackgroundColor"];
                BackgroundColor.AdvancedSearch.Save();
            }
            ...
            // Field str_AppointmentType
            if (filter?.TryGetValue("x_str_AppointmentType", out sv) ?? false) {
                str_AppointmentType.AdvancedSearch.SearchValue = sv;
                str_AppointmentType.AdvancedSearch.SearchOperator = filter["z_str_AppointmentType"];
                str_AppointmentType.AdvancedSearch.SearchCondition = filter["v_str_AppointmentType"];
                str_AppointmentType.AdvancedSearch.SearchValue2 = filter["y_str_AppointmentType"];
                str_AppointmentType.AdvancedSearch.SearchOperator2 = filter["w_str_AppointmentType"];
                str_AppointmentType.AdvancedSearch.Save();
            }

            // Field str_AppointmentStatus
            if (filter?.TryGetValue("x_str_AppointmentStatus", out sv) ?? false) {
                str_AppointmentStatus.AdvancedSearch.SearchValue = sv;
                str_AppointmentStatus.AdvancedSearch.SearchOperator = filter["z_str_AppointmentStatus"];
                str_AppointmentStatus.AdvancedSearch.SearchCondition = filter["v_str_AppointmentStatus"];
                str_AppointmentStatus.AdvancedSearch.SearchValue2 = filter["y_str_AppointmentStatus"];
                str_AppointmentStatus.AdvancedSearch.SearchOperator2 = filter["w_str_AppointmentStatus"];
                str_AppointmentStatus.AdvancedSearch.Save();
            }
            ...
            // Field str_CRSS_IDUSER
            if (filter?.TryGetValue("x_str_CRSS_IDUSER", out sv) ?? false) {
                str_CRSS_IDUSER.AdvancedSearch.SearchValue = sv;
                str_CRSS_IDUSER.AdvancedSearch.SearchOperator = filter["z_str_CRSS_IDUSER"];
                str_CRSS_IDUSER.AdvancedSearch.SearchCondition = filter["v_str_CRSS_IDUSER"];
                str_CRSS_IDUSER.AdvancedSearch.SearchValue2 = filter["y_str_CRSS_IDUSER"];
                str_CRSS_IDUSER.AdvancedSearch.SearchOperator2 = filter["w_str_CRSS_IDUSER"];
                str_CRSS_IDUSER.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(_Title);
            searchFlds.Add(Description);
            searchFlds.Add(_Url);
            searchFlds.Add(Display);
            searchFlds.Add(BackgroundColor);
            searchFlds.Add(CRSS_ID);
            searchFlds.Add(str_AppointmentType);
            searchFlds.Add(str_AppointmentStatus);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(int_Enrollement_Id, ctrl); // int_Enrollement_Id
                UpdateSort(int_PackageID, ctrl); // int_PackageID
                UpdateSort(Start, ctrl); // Start
                UpdateSort(End, ctrl); // End
                UpdateSort(str_AppointmentType, ctrl); // str_AppointmentType
                UpdateSort(str_AppointmentStatus, ctrl); // str_AppointmentStatus
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    CRSS_ID.Sort = "";
                    str_AppointmentType.Sort = "";
                    str_AppointmentStatus.Sort = "";
                    str_Username.Sort = "";
                    str_CRSS_IDUSER.Sort = "";
                }
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""Appointment_Cldr"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""Appointment_Cldr"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""Appointment_Cldr"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fAppointment_Cldrlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fAppointment_Cldrlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(Id.CurrentValue) + "\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""Appointment_Cldr"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fAppointment_Cldrlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fAppointment_Cldrsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fAppointment_Cldrsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fAppointment_Cldrlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_Appointment_Cldr");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(appointmentCldrList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(appointmentCldrList.RowCount) + "_Appointment_Cldr");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(appointmentCldrList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && appointmentCldrList.RowType == RowType.Add || IsEdit && appointmentCldrList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fAppointment_Cldrsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
