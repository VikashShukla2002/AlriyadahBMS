...
public partial class JUN26 {
    /// <summary>
    /// tblRptMgrInvoicesAdd
    /// </summary>
    public static TblRptMgrInvoicesAdd tblRptMgrInvoicesAdd
    {
        get => HttpData.Get<TblRptMgrInvoicesAdd>("tblRptMgrInvoicesAdd")!;
        set => HttpData["tblRptMgrInvoicesAdd"] = value;
    }

    /// <summary>
    /// Page class for tblRptMgrInvoices
    /// </summary>
    public class TblRptMgrInvoicesAdd : TblRptMgrInvoicesAddBase
    {
        // Constructor
        public TblRptMgrInvoicesAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblRptMgrInvoicesAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TblRptMgrInvoicesAddBase : TblRptMgrInvoices
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "tblRptMgrInvoicesAdd";
        ...
        // Constructor
        public TblRptMgrInvoicesAddBase()
        {
            // Initialize
            CurrentPage = this;
        ...
        // Page name
        public string PageName => "TblRptMgrInvoicesAdd";
        ...
        // Constructor
        public TblRptMgrInvoicesAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("RptMgr_ID", out rv)) { // DN
                    RptMgr_ID.QueryValue = UrlDecode(rv); // DN
                } else if (Get("RptMgr_ID", out sv)) {
                    RptMgr_ID.QueryValue = sv.ToString();
                }
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("TblRptMgrInvoicesList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        string returnUrl = "";
                        returnUrl = ViewUrl;
                        if (GetPageName(returnUrl) == "TblRptMgrInvoicesList")
                            returnUrl = AddMasterUrl(ListUrl); // List page, return to List page with correct master key if necessary
                        else if (GetPageName(returnUrl) == "TblRptMgrInvoicesView")
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "TblRptMgrInvoicesList") {
                                ...
                                returnUrl = "TblRptMgrInvoicesList"; // Return list page content
                            }
                        }
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                tblRptMgrInvoicesAdd?.PageRender();
            }
            return PageResult();
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'Program_ID' before field var 'x_Program_ID'
            val = CurrentForm.HasValue("Program_ID") ? CurrentForm.GetValue("Program_ID") : CurrentForm.GetValue("x_Program_ID");
            if (!Program_ID.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Program_ID") && !CurrentForm.HasValue("x_Program_ID")) // DN
                    Program_ID.Visible = false; // Disable update for API request
                else
                    Program_ID.SetFormValue(val);
            }

            // Check field name 'Report_Date' before field var 'x_Report_Date'
            val = CurrentForm.HasValue("Report_Date") ? CurrentForm.GetValue("Report_Date") : CurrentForm.GetValue("x_Report_Date");
            if (!Report_Date.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Report_Date") && !CurrentForm.HasValue("x_Report_Date")) // DN
                    Report_Date.Visible = false; // Disable update for API request
                else
                    Report_Date.SetFormValue(val, true, validate);
                Report_Date.CurrentValue = UnformatDateTime(Report_Date.CurrentValue, Report_Date.FormatPattern);
            }
            ...
            // Check field name 'RptMgr_ID' before field var 'x_RptMgr_ID'
            val = CurrentForm.HasValue("RptMgr_ID") ? CurrentForm.GetValue("RptMgr_ID") : CurrentForm.GetValue("x_RptMgr_ID");
        }
        #pragma warning restore 1998
        ...
        public void RestoreFormValues()
        {
            ...
            Program_ID.CurrentValue = Program_ID.FormValue;
            Report_Date.CurrentValue = Report_Date.FormValue;
            Report_Date.CurrentValue = UnformatDateTime(Report_Date.CurrentValue, Report_Date.FormatPattern);
        ...
        public async Task RenderRow()
        {
            ...
            // RptMgr_ID
            RptMgr_ID.RowCssClass = "row";

            // NationalID
            NationalID.RowCssClass = "row";

            // Program_ID
            Program_ID.RowCssClass = "row";

            // Report_Date
            Report_Date.RowCssClass = "row";

            // str_Username
            str_Username.RowCssClass = "row";

            // Report_Link
            Report_Link.RowCssClass = "row";

            // Report
            Report.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                curVal = ConvertToString(Program_ID.CurrentValue)?.Trim() ?? "";
                if (Program_ID.Lookup != null && IsDictionary(Program_ID.Lookup?.Options) && Program_ID.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    Program_ID.EditValue = Program_ID.Lookup?.Options.Values.ToList();
                } else { // Lookup from database
                    if (curVal == "") {
                        filterWrk = "0=1";
                    } else {
                    ...
                    sqlWrk = Program_ID.Lookup?.GetSql(true, filterWrk, null, this, false, true);
                    rswrk = sqlWrk != null ? Connection.GetRows(sqlWrk) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    Program_ID.EditValue = rswrk;
                }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (Program_ID.Required) {
                if (!Program_ID.IsDetailKey && Empty(Program_ID.FormValue)) {
                    Program_ID.AddErrorMessage(ConvertToString(Program_ID.RequiredErrorMessage).Replace("%s", Program_ID.Caption));
                }
            }
            if (Report_Date.Required) {
                if (!Report_Date.IsDetailKey && Empty(Report_Date.FormValue)) {
                    Report_Date.AddErrorMessage(ConvertToString(Report_Date.RequiredErrorMessage).Replace("%s", Report_Date.Caption));
                }
            }
            if (!CheckDate(Report_Date.FormValue, Report_Date.FormatPattern)) {
                Report_Date.AddErrorMessage(Report_Date.GetErrorMessage(false));
            }
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                ...
                // Program_ID
                Program_ID.SetDbValue(rsnew, Program_ID.CurrentValue);

                // Report_Date
                Report_Date.SetDbValue(rsnew, ConvertToDateTime(Report_Date.CurrentValue, Report_Date.FormatPattern));
            ...
            if (insertRow) {
                try {
                    ...
                    rsnew["RptMgr_ID"] = RptMgr_ID.CurrentValue!;
                } catch (Exception e) {
                    if (Config.Debug)
                        throw;
                    FailureMessage = e.Message;
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("TblRptMgrInvoicesList")), "", TableVar, true);
