...
public partial class JUN26 {
    ...
    /// dshbdClassesList
    /// </summary>
    public static DshbdClassesList dshbdClassesList
    {
        get => HttpData.Get<DshbdClassesList>("dshbdClassesList")!;
        set => HttpData["dshbdClassesList"] = value;
    }

    /// <summary>
    /// Page class for dshbdClasses
    /// </summary>
    public class DshbdClassesList : DshbdClassesListBase
    {
        // Constructor
        public DshbdClassesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public DshbdClassesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class DshbdClassesListBase : DshbdClasses
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "dshbdClasses";

        // Page object name
        public string PageObjName = "dshbdClassesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fdshbdClasseslist";
        ...
        // Constructor
        public DshbdClassesListBase()
        {
            // CSS class name as context
            TableVar = "dshbdClasses";
            ...
            // Table object (dshbdClasses)
            if (dshbdClasses == null || dshbdClasses is DshbdClasses)
                dshbdClasses = this;

            // Initialize URLs
            AddUrl = "DshbdClassesAdd";
            ...
            MultiDeleteUrl = "DshbdClassesDelete";
            MultiUpdateUrl = "DshbdClassesUpdate";
        ...
        // Page name
        public string PageName => "DshbdClassesList";
        ...
        {
            ClassStatus.SetVisibility();
            str_Package_Name.SetVisibility();
            Numberofopenclasses.SetVisibility();
        }

        // Constructor
        public DshbdClassesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "DshbdClassesView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected void HideFieldsForAddEdit() {
            if (IsAddOrEdit)
                ClassStatus.Visible = false;
            if (IsAddOrEdit)
                Numberofopenclasses.Visible = false;
        }
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fdshbdClassessrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(ClassStatus.AdvancedSearch.ToJson())); // Field Class Status
            filters.Merge(JObject.Parse(str_Package_Name.AdvancedSearch.ToJson())); // Field str_Package_Name
            filters.Merge(JObject.Parse(Numberofopenclasses.AdvancedSearch.ToJson())); // Field Number of open classes
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fdshbdClassessrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Class Status
            if (filter?.TryGetValue("x_ClassStatus", out sv) ?? false) {
                ClassStatus.AdvancedSearch.SearchValue = sv;
                ClassStatus.AdvancedSearch.SearchOperator = filter["z_ClassStatus"];
                ClassStatus.AdvancedSearch.SearchCondition = filter["v_ClassStatus"];
                ClassStatus.AdvancedSearch.SearchValue2 = filter["y_ClassStatus"];
                ClassStatus.AdvancedSearch.SearchOperator2 = filter["w_ClassStatus"];
                ClassStatus.AdvancedSearch.Save();
            }
            ...
            // Field Number of open classes
            if (filter?.TryGetValue("x_Numberofopenclasses", out sv) ?? false) {
                Numberofopenclasses.AdvancedSearch.SearchValue = sv;
                Numberofopenclasses.AdvancedSearch.SearchOperator = filter["z_Numberofopenclasses"];
                Numberofopenclasses.AdvancedSearch.SearchCondition = filter["v_Numberofopenclasses"];
                Numberofopenclasses.AdvancedSearch.SearchValue2 = filter["y_Numberofopenclasses"];
                Numberofopenclasses.AdvancedSearch.SearchOperator2 = filter["w_Numberofopenclasses"];
                Numberofopenclasses.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(ClassStatus, ctrl); // Class Status
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdClasseslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdClasseslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fdshbdClasseslist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fdshbdClassessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fdshbdClassessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdClasseslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_dshbdClasses");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(dshbdClassesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(dshbdClassesList.RowCount) + "_dshbdClasses");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(dshbdClassesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && dshbdClassesList.RowType == RowType.Add || IsEdit && dshbdClassesList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RowSelected(row);
            ClassStatus.SetDbValue(row["Class Status"]);
            str_Package_Name.SetDbValue(row["str_Package_Name"]);
            Numberofopenclasses.SetDbValue(row["Number of open classes"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("Class Status", ClassStatus.DefaultValue ?? DbNullValue); // DN
            row.Add("str_Package_Name", str_Package_Name.DefaultValue ?? DbNullValue); // DN
            row.Add("Number of open classes", Numberofopenclasses.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // Class Status
                ClassStatus.ViewValue = ConvertToString(ClassStatus.CurrentValue); // DN
                ClassStatus.ViewCustomAttributes = "";
                ...
                // Class Status
                ClassStatus.HrefValue = "";
                ClassStatus.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fdshbdClassessrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
