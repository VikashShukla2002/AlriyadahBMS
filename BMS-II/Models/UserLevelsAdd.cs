...
public partial class JUN26 {
    ...
    /// userLevelsAdd
    /// </summary>
    public static UserLevelsAdd userLevelsAdd
    {
        get => HttpData.Get<UserLevelsAdd>("userLevelsAdd")!;
        set => HttpData["userLevelsAdd"] = value;
    }

    /// <summary>
    /// Page class for UserLevels
    /// </summary>
    public class UserLevelsAdd : UserLevelsAddBase
    {
        // Constructor
        public UserLevelsAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public UserLevelsAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class UserLevelsAddBase : UserLevels
    {
        // Page ID
        ...
        // Page name
        public string PageName => "UserLevelsAdd";
        ...
        // Constructor
        public UserLevelsAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("UserLevelID", out rv)) { // DN
                    UserLevelID.QueryValue = UrlDecode(rv); // DN
                } else if (Get("UserLevelID", out sv)) {
            ...
            if (postBack) {
                ...
                foreach (string privilege in Config.Privileges)
                    priv += CurrentForm.GetInt("x__Allow" + TitleCaseInvariant(privilege));
                if (!IsSysAdmin())
                    priv -= CurrentForm.GetInt("x_AllowAdmin");
            }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'UserLevelName' before field var 'x_UserLevelName'
            ...
            if (!UserLevelName.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("UserLevelName") && !CurrentForm.HasValue("x_UserLevelName")) // DN
                    UserLevelName.Visible = false; // Disable update for API request
                else
                    UserLevelName.SetFormValue(val);
            }
        }
        #pragma warning restore 1998
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // UserLevelName
                UserLevelName.SetupEditAttributes();
                if (!UserLevelName.Raw)
                    UserLevelName.CurrentValue = HtmlDecode(UserLevelName.CurrentValue);
                UserLevelName.EditValue = HtmlEncode(UserLevelName.CurrentValue);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (UserLevelName.Required) {
                if (!UserLevelName.IsDetailKey && Empty(UserLevelName.FormValue)) {
                    UserLevelName.AddErrorMessage(ConvertToString(UserLevelName.RequiredErrorMessage).Replace("%s", UserLevelName.Caption));
                }
            }
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                ...
                // UserLevelName
                UserLevelName.SetDbValue(rsnew, UserLevelName.CurrentValue);
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
            ...
            if (UserLevelID.Required && Empty(UserLevelID.CurrentValue)) {
                FailureMessage = Language.Phrase("MissingUserLevelID");
            } else if (Empty(UserLevelName.CurrentValue)) {
            ...
            } else if (ConvertToInt(UserLevelID.CurrentValue) == 0 && !SameText(UserLevelName.CurrentValue, "default")) {
                FailureMessage = Language.Phrase("UserLevelDefaultName");
            } else if (ConvertToInt(UserLevelID.CurrentValue) == -1 && !SameText(UserLevelName.CurrentValue, "administrator")) {
                FailureMessage = Language.Phrase("UserLevelAdministratorName");
            } else if (ConvertToInt(UserLevelID.CurrentValue) == -2 && !SameText(UserLevelName.CurrentValue, "anonymous")) {
                FailureMessage = Language.Phrase("UserLevelAnonymousName");
            } else if (ConvertToInt(UserLevelID.CurrentValue) > 0 && (SameText(UserLevelName.CurrentValue, "administrator") || SameText(UserLevelName.CurrentValue, "default") || SameText(UserLevelName.CurrentValue, "anonymous"))) {
            ...
            if (!Empty(UserLevelID.CurrentValue)) { // Check field with unique index
                ...
                if (rschk?.HasRows ?? false) { // DN
                    FailureMessage = Language.Phrase("DupIndex").Replace("%f", UserLevelID.Caption).Replace("%v", ConvertToString(UserLevelID.CurrentValue));
                    return JsonBoolResult.FalseResult;
                }
            }
            ...
            // Add User Level priv
            if (priv > 0) {
                var userLevelList = Config.UserLevels;
                var userLevelPrivList = Config.UserLevelPermissions;
                var tableList = Config.UserLevelTablePermissions;
                var columns = new [] { Config.UserLevelPrivTableNameField, Config.UserLevelPrivUserLevelIdField, Config.UserLevelPrivPrivField };
                var valuesCollection = tableList.Select(item => new object[] {
                    item.ProjectId + item.TableName,
                    ConvertToInt(UserLevelID.CurrentValue),
                    priv
                });
                await Resolve(Config.UserLevelPrivTableName)?.GetQueryBuilder(true).InsertAsync(columns, valuesCollection); // Use main connection
            }
