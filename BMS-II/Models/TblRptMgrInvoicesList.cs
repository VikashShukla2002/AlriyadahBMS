...
public partial class JUN26 {
    /// <summary>
    /// tblRptMgrInvoicesList
    /// </summary>
    public static TblRptMgrInvoicesList tblRptMgrInvoicesList
    {
        get => HttpData.Get<TblRptMgrInvoicesList>("tblRptMgrInvoicesList")!;
        set => HttpData["tblRptMgrInvoicesList"] = value;
    }

    /// <summary>
    /// Page class for tblRptMgrInvoices
    /// </summary>
    public class TblRptMgrInvoicesList : TblRptMgrInvoicesListBase
    {
        // Constructor
        public TblRptMgrInvoicesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblRptMgrInvoicesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TblRptMgrInvoicesListBase : TblRptMgrInvoices
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "tblRptMgrInvoices";

        // Page object name
        public string PageObjName = "tblRptMgrInvoicesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ftblRptMgrInvoiceslist";
        ...
        // Constructor
        public TblRptMgrInvoicesListBase()
        {
            ...
            // Table object (tblRptMgrInvoices)
            if (tblRptMgrInvoices == null || tblRptMgrInvoices is TblRptMgrInvoices)
                tblRptMgrInvoices = this;

            // Initialize URLs
            AddUrl = "TblRptMgrInvoicesAdd";
            ...
            MultiDeleteUrl = "TblRptMgrInvoicesDelete";
            MultiUpdateUrl = "TblRptMgrInvoicesUpdate";
        ...
        // Page name
        public string PageName => "TblRptMgrInvoicesList";
        ...
        {
            RptMgr_ID.Visible = false;
            NationalID.SetVisibility();
            Program_ID.Visible = false;
            Report_Date.SetVisibility();
            str_Username.SetVisibility();
            Report_Link.SetVisibility();
            Report.SetVisibility();
        }

        // Constructor
        public TblRptMgrInvoicesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "TblRptMgrInvoicesView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("RptMgr_ID") ? dict["RptMgr_ID"] : RptMgr_ID.CurrentValue));
            return key;
        }
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            await SetupLookupOptions(Program_ID);

            // Update form name to avoid conflict
            if (IsModal)
                FormName = "ftblRptMgrInvoicesgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                tblRptMgrInvoicesList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "ftblRptMgrInvoicessrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(RptMgr_ID.AdvancedSearch.ToJson())); // Field RptMgr_ID
            filters.Merge(JObject.Parse(NationalID.AdvancedSearch.ToJson())); // Field NationalID
            filters.Merge(JObject.Parse(Program_ID.AdvancedSearch.ToJson())); // Field Program_ID
            filters.Merge(JObject.Parse(Report_Date.AdvancedSearch.ToJson())); // Field Report_Date
            filters.Merge(JObject.Parse(str_Username.AdvancedSearch.ToJson())); // Field str_Username
            filters.Merge(JObject.Parse(Report_Link.AdvancedSearch.ToJson())); // Field Report_Link
            filters.Merge(JObject.Parse(Report.AdvancedSearch.ToJson())); // Field Report
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "ftblRptMgrInvoicessrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field RptMgr_ID
            if (filter?.TryGetValue("x_RptMgr_ID", out sv) ?? false) {
                RptMgr_ID.AdvancedSearch.SearchValue = sv;
                RptMgr_ID.AdvancedSearch.SearchOperator = filter["z_RptMgr_ID"];
                RptMgr_ID.AdvancedSearch.SearchCondition = filter["v_RptMgr_ID"];
                RptMgr_ID.AdvancedSearch.SearchValue2 = filter["y_RptMgr_ID"];
                RptMgr_ID.AdvancedSearch.SearchOperator2 = filter["w_RptMgr_ID"];
                RptMgr_ID.AdvancedSearch.Save();
            }
            ...
            // Field Program_ID
            if (filter?.TryGetValue("x_Program_ID", out sv) ?? false) {
                Program_ID.AdvancedSearch.SearchValue = sv;
                Program_ID.AdvancedSearch.SearchOperator = filter["z_Program_ID"];
                Program_ID.AdvancedSearch.SearchCondition = filter["v_Program_ID"];
                Program_ID.AdvancedSearch.SearchValue2 = filter["y_Program_ID"];
                Program_ID.AdvancedSearch.SearchOperator2 = filter["w_Program_ID"];
                Program_ID.AdvancedSearch.Save();
            }

            // Field Report_Date
            if (filter?.TryGetValue("x_Report_Date", out sv) ?? false) {
                Report_Date.AdvancedSearch.SearchValue = sv;
                Report_Date.AdvancedSearch.SearchOperator = filter["z_Report_Date"];
                Report_Date.AdvancedSearch.SearchCondition = filter["v_Report_Date"];
                Report_Date.AdvancedSearch.SearchValue2 = filter["y_Report_Date"];
                Report_Date.AdvancedSearch.SearchOperator2 = filter["w_Report_Date"];
                Report_Date.AdvancedSearch.Save();
            }
            ...
            // Field Report_Link
            if (filter?.TryGetValue("x_Report_Link", out sv) ?? false) {
                Report_Link.AdvancedSearch.SearchValue = sv;
                Report_Link.AdvancedSearch.SearchOperator = filter["z_Report_Link"];
                Report_Link.AdvancedSearch.SearchCondition = filter["v_Report_Link"];
                Report_Link.AdvancedSearch.SearchValue2 = filter["y_Report_Link"];
                Report_Link.AdvancedSearch.SearchOperator2 = filter["w_Report_Link"];
                Report_Link.AdvancedSearch.Save();
            }

            // Field Report
            if (filter?.TryGetValue("x_Report", out sv) ?? false) {
                Report.AdvancedSearch.SearchValue = sv;
                Report.AdvancedSearch.SearchOperator = filter["z_Report"];
                Report.AdvancedSearch.SearchCondition = filter["v_Report"];
                Report.AdvancedSearch.SearchValue2 = filter["y_Report"];
                Report.AdvancedSearch.SearchOperator2 = filter["w_Report"];
                Report.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Report_Link);
            searchFlds.Add(Report);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Report_Date, ctrl); // Report_Date
                UpdateSort(str_Username, ctrl); // str_Username
                UpdateSort(Report_Link, ctrl); // Report_Link
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""tblRptMgrInvoices"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblRptMgrInvoiceslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblRptMgrInvoiceslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(RptMgr_ID.CurrentValue) + "\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""tblRptMgrInvoices"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ftblRptMgrInvoiceslist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ftblRptMgrInvoicessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ftblRptMgrInvoicessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ftblRptMgrInvoiceslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_tblRptMgrInvoices");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(tblRptMgrInvoicesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(tblRptMgrInvoicesList.RowCount) + "_tblRptMgrInvoices");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(tblRptMgrInvoicesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && tblRptMgrInvoicesList.RowType == RowType.Add || IsEdit && tblRptMgrInvoicesList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RowSelected(row);
            RptMgr_ID.SetDbValue(row["RptMgr_ID"]);
            NationalID.SetDbValue(row["NationalID"]);
            Program_ID.SetDbValue(row["Program_ID"]);
            Report_Date.SetDbValue(row["Report_Date"]);
            str_Username.SetDbValue(row["str_Username"]);
            Report_Link.SetDbValue(row["Report_Link"]);
            Report.SetDbValue(row["Report"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("RptMgr_ID", RptMgr_ID.DefaultValue ?? DbNullValue); // DN
            row.Add("NationalID", NationalID.DefaultValue ?? DbNullValue); // DN
            row.Add("Program_ID", Program_ID.DefaultValue ?? DbNullValue); // DN
            row.Add("Report_Date", Report_Date.DefaultValue ?? DbNullValue); // DN
            row.Add("str_Username", str_Username.DefaultValue ?? DbNullValue); // DN
            row.Add("Report_Link", Report_Link.DefaultValue ?? DbNullValue); // DN
            row.Add("Report", Report.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ftblRptMgrInvoicessrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
