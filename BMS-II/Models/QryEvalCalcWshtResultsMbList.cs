...
public partial class JUN26 {
    /// <summary>
    /// qryEvalCalcWshtResultsMbList
    /// </summary>
    public static QryEvalCalcWshtResultsMbList qryEvalCalcWshtResultsMbList
    {
        get => HttpData.Get<QryEvalCalcWshtResultsMbList>("qryEvalCalcWshtResultsMbList")!;
        set => HttpData["qryEvalCalcWshtResultsMbList"] = value;
    }

    /// <summary>
    /// Page class for qryEvalCalcWsht_ResultsMB
    /// </summary>
    public class QryEvalCalcWshtResultsMbList : QryEvalCalcWshtResultsMbListBase
    {
        // Constructor
        public QryEvalCalcWshtResultsMbList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public QryEvalCalcWshtResultsMbList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class QryEvalCalcWshtResultsMbListBase : QryEvalCalcWshtResultsMb
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "qryEvalCalcWsht_ResultsMB";

        // Page object name
        public string PageObjName = "qryEvalCalcWshtResultsMbList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fqryEvalCalcWsht_ResultsMBlist";
        ...
        // Constructor
        public QryEvalCalcWshtResultsMbListBase()
        {
            ...
            // Table object (qryEvalCalcWshtResultsMb)
            if (qryEvalCalcWshtResultsMb == null || qryEvalCalcWshtResultsMb is QryEvalCalcWshtResultsMb)
                qryEvalCalcWshtResultsMb = this;

            // Initialize URLs
            AddUrl = "QryEvalCalcWshtResultsMbAdd";
            ...
            MultiDeleteUrl = "QryEvalCalcWshtResultsMbDelete";
            MultiUpdateUrl = "QryEvalCalcWshtResultsMbUpdate";
        ...
        // Page name
        public string PageName => "QryEvalCalcWshtResultsMbList";
        ...
        public void SetVisibility()
        {
            ...
            Sect11Sum.SetVisibility();
            Subtotal.SetVisibility();
            Assign_int_Package_Id.SetVisibility();
            Package_Name.SetVisibility();
            Discount.SetVisibility();
            Price.SetVisibility();
            Package_Items.SetVisibility();
        ...
        // Constructor
        public QryEvalCalcWshtResultsMbListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "QryEvalCalcWshtResultsMbView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected void HideFieldsForAddEdit() {
            ...
            if (IsAddOrEdit)
                Sect11Sum.Visible = false;
            if (IsAddOrEdit)
                Subtotal.Visible = false;
            if (IsAddOrEdit)
                Assign_int_Package_Id.Visible = false;
            if (IsAddOrEdit)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fqryEvalCalcWsht_ResultsMBgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                qryEvalCalcWshtResultsMbList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fqryEvalCalcWsht_ResultsMBsrch");
            ...
            filters.Merge(JObject.Parse(Sect11Sum.AdvancedSearch.ToJson())); // Field Sect11Sum
            filters.Merge(JObject.Parse(Subtotal.AdvancedSearch.ToJson())); // Field Subtotal
            filters.Merge(JObject.Parse(Assign_int_Package_Id.AdvancedSearch.ToJson())); // Field Assign_int_Package_Id
            filters.Merge(JObject.Parse(Package_Name.AdvancedSearch.ToJson())); // Field Package_Name
            filters.Merge(JObject.Parse(Discount.AdvancedSearch.ToJson())); // Field Discount
            filters.Merge(JObject.Parse(Price.AdvancedSearch.ToJson())); // Field Price
            filters.Merge(JObject.Parse(Package_Items.AdvancedSearch.ToJson())); // Field Package_Items
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fqryEvalCalcWsht_ResultsMBsrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Sect11Sum
            if (filter?.TryGetValue("x_Sect11Sum", out sv) ?? false) {
                Sect11Sum.AdvancedSearch.SearchValue = sv;
                Sect11Sum.AdvancedSearch.SearchOperator = filter["z_Sect11Sum"];
                Sect11Sum.AdvancedSearch.SearchCondition = filter["v_Sect11Sum"];
                Sect11Sum.AdvancedSearch.SearchValue2 = filter["y_Sect11Sum"];
                Sect11Sum.AdvancedSearch.SearchOperator2 = filter["w_Sect11Sum"];
                Sect11Sum.AdvancedSearch.Save();
            }

            // Field Subtotal
            if (filter?.TryGetValue("x_Subtotal", out sv) ?? false) {
                Subtotal.AdvancedSearch.SearchValue = sv;
                Subtotal.AdvancedSearch.SearchOperator = filter["z_Subtotal"];
                Subtotal.AdvancedSearch.SearchCondition = filter["v_Subtotal"];
                Subtotal.AdvancedSearch.SearchValue2 = filter["y_Subtotal"];
                Subtotal.AdvancedSearch.SearchOperator2 = filter["w_Subtotal"];
                Subtotal.AdvancedSearch.Save();
            }

            // Field Assign_int_Package_Id
            if (filter?.TryGetValue("x_Assign_int_Package_Id", out sv) ?? false) {
                Assign_int_Package_Id.AdvancedSearch.SearchValue = sv;
                Assign_int_Package_Id.AdvancedSearch.SearchOperator = filter["z_Assign_int_Package_Id"];
                Assign_int_Package_Id.AdvancedSearch.SearchCondition = filter["v_Assign_int_Package_Id"];
                Assign_int_Package_Id.AdvancedSearch.SearchValue2 = filter["y_Assign_int_Package_Id"];
                Assign_int_Package_Id.AdvancedSearch.SearchOperator2 = filter["w_Assign_int_Package_Id"];
                Assign_int_Package_Id.AdvancedSearch.Save();
            }
            ...
            // Field Discount
            if (filter?.TryGetValue("x_Discount", out sv) ?? false) {
                Discount.AdvancedSearch.SearchValue = sv;
                Discount.AdvancedSearch.SearchOperator = filter["z_Discount"];
                Discount.AdvancedSearch.SearchCondition = filter["v_Discount"];
                Discount.AdvancedSearch.SearchValue2 = filter["y_Discount"];
                Discount.AdvancedSearch.SearchOperator2 = filter["w_Discount"];
                Discount.AdvancedSearch.Save();
            }
            ...
            // Field Package_Items
            if (filter?.TryGetValue("x_Package_Items", out sv) ?? false) {
                Package_Items.AdvancedSearch.SearchValue = sv;
                Package_Items.AdvancedSearch.SearchOperator = filter["z_Package_Items"];
                Package_Items.AdvancedSearch.SearchCondition = filter["v_Package_Items"];
                Package_Items.AdvancedSearch.SearchValue2 = filter["y_Package_Items"];
                Package_Items.AdvancedSearch.SearchOperator2 = filter["w_Package_Items"];
                Package_Items.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Discount);
            searchFlds.Add(Package_Items);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Eval_ID, ctrl); // Eval_ID
                UpdateSort(int_Student_ID, ctrl); // int_Student_ID
                UpdateSort(AssessmentID, ctrl); // AssessmentID
                ...
                UpdateSort(Date_Entered, ctrl); // Date_Entered
                UpdateSort(Examination_Number, ctrl); // Examination_Number
                UpdateSort(Sect11Sum, ctrl); // Sect11Sum
                UpdateSort(Subtotal, ctrl); // Subtotal
                UpdateSort(Assign_int_Package_Id, ctrl); // Assign_int_Package_Id
                UpdateSort(Package_Name, ctrl); // Package_Name
                UpdateSort(Discount, ctrl); // Discount
                UpdateSort(Price, ctrl); // Price
                UpdateSort(Package_Items, ctrl); // Package_Items
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryEvalCalcWsht_ResultsMBlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryEvalCalcWsht_ResultsMBlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fqryEvalCalcWsht_ResultsMBlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fqryEvalCalcWsht_ResultsMBsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fqryEvalCalcWsht_ResultsMBsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fqryEvalCalcWsht_ResultsMBlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_qryEvalCalcWsht_ResultsMB");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(qryEvalCalcWshtResultsMbList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(qryEvalCalcWshtResultsMbList.RowCount) + "_qryEvalCalcWsht_ResultsMB");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(qryEvalCalcWshtResultsMbList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && qryEvalCalcWshtResultsMbList.RowType == RowType.Add || IsEdit && qryEvalCalcWshtResultsMbList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Sect11Sum.SetDbValue(row["Sect11Sum"]);
            Subtotal.SetDbValue(row["Subtotal"]);
            Assign_int_Package_Id.SetDbValue(row["Assign_int_Package_Id"]);
            Package_Name.SetDbValue(row["Package_Name"]);
            Discount.SetDbValue(row["Discount"]);
            Price.SetDbValue(row["Price"]);
            Package_Items.SetDbValue(row["Package_Items"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Sect11Sum", Sect11Sum.DefaultValue ?? DbNullValue); // DN
            row.Add("Subtotal", Subtotal.DefaultValue ?? DbNullValue); // DN
            row.Add("Assign_int_Package_Id", Assign_int_Package_Id.DefaultValue ?? DbNullValue); // DN
            row.Add("Package_Name", Package_Name.DefaultValue ?? DbNullValue); // DN
            row.Add("Discount", Discount.DefaultValue ?? DbNullValue); // DN
            row.Add("Price", Price.DefaultValue ?? DbNullValue); // DN
            row.Add("Package_Items", Package_Items.DefaultValue ?? DbNullValue); // DN
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fqryEvalCalcWsht_ResultsMBsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
