...
public partial class JUN26 {
    ...
    /// userLevelsList
    /// </summary>
    public static UserLevelsList userLevelsList
    {
        get => HttpData.Get<UserLevelsList>("userLevelsList")!;
        set => HttpData["userLevelsList"] = value;
    }

    /// <summary>
    /// Page class for UserLevels
    /// </summary>
    public class UserLevelsList : UserLevelsListBase
    {
        // Constructor
        public UserLevelsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public UserLevelsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class UserLevelsListBase : UserLevels
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "UserLevels";
        ...
        public UserLevelsListBase()
        {
            ...
            MultiDeleteUrl = "UserLevelsDelete";
            MultiUpdateUrl = "UserLevelsUpdate";
        ...
        // Page name
        public string PageName => "UserLevelsList";
        ...
        // Constructor
        public UserLevelsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "UserLevelsView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("UserLevelID") ? dict["UserLevelID"] : UserLevelID.CurrentValue));
            return key;
        }
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fUserLevelssrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fUserLevelssrch", filters);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(UserLevelName, ctrl); // UserLevelName
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""UserLevels"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            // "userpermission"
            listOption = ListOptions["userpermission"];
            if (Security.HasUserLevelID(ConvertToInt(UserLevelID.CurrentValue)) || ConvertToInt(UserLevelID.CurrentValue) < 0 && ConvertToInt(UserLevelID.CurrentValue) != -2) {
                listOption?.SetBody("-");
            } else {
                listOption?.SetBody("<a class=\"ew-row-link ew-user-permission\" title=\"" + Language.Phrase("Permission", true) + "\" data-caption=\"" + Language.Phrase("Permission", true) + "\" href=\"" + HtmlEncode(AppPath("userpriv?UserLevelID=" + UserLevelID.CurrentValue)) + "\">" + Language.Phrase("Permission") + "</a>");
            }

            // "checkbox"
            listOption = ListOptions["checkbox"];
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(UserLevelID.CurrentValue) + "\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""UserLevels"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fUserLevelslist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fUserLevelssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fUserLevelssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(userLevelsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(userLevelsList.RowCount) + "_UserLevels");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(userLevelsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && userLevelsList.RowType == RowType.Add || IsEdit && userLevelsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("UserLevelName", UserLevelName.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // UserLevelName
                UserLevelName.ViewValue = ConvertToString(UserLevelName.CurrentValue); // DN
                var _name = ResolveSecurity().GetUserLevelName(ConvertToString(UserLevelID.CurrentValue)); // DN
                if (!Empty(_name))
                    UserLevelName.ViewValue = _name;
                UserLevelName.ViewCustomAttributes = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fUserLevelssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
