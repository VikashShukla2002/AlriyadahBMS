...
public partial class JUN26 {
    ...
    /// dshbdRegisteredList
    /// </summary>
    public static DshbdRegisteredList dshbdRegisteredList
    {
        get => HttpData.Get<DshbdRegisteredList>("dshbdRegisteredList")!;
        set => HttpData["dshbdRegisteredList"] = value;
    }

    /// <summary>
    /// Page class for dshbdRegistered
    /// </summary>
    public class DshbdRegisteredList : DshbdRegisteredListBase
    {
        // Constructor
        public DshbdRegisteredList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public DshbdRegisteredList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class DshbdRegisteredListBase : DshbdRegistered
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "dshbdRegistered";

        // Page object name
        public string PageObjName = "dshbdRegisteredList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fdshbdRegisteredlist";
        ...
        // Constructor
        public DshbdRegisteredListBase()
        {
            // CSS class name as context
            TableVar = "dshbdRegistered";
            ...
            // Table object (dshbdRegistered)
            if (dshbdRegistered == null || dshbdRegistered is DshbdRegistered)
                dshbdRegistered = this;

            // Initialize URLs
            AddUrl = "DshbdRegisteredAdd";
            ...
            MultiDeleteUrl = "DshbdRegisteredDelete";
            MultiUpdateUrl = "DshbdRegisteredUpdate";
        ...
        // Page name
        public string PageName => "DshbdRegisteredList";
        ...
        public void SetVisibility()
        {
            ...
            RegistrationDate.SetVisibility();
            NationalID.SetVisibility();
            str_Username.SetVisibility();
            DrivingExperience.SetVisibility();
        }

        // Constructor
        public DshbdRegisteredListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "DshbdRegisteredView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected void HideFieldsForAddEdit() {
            if (IsAddOrEdit)
                RegistrationDate.Visible = false;
            if (IsAddOrEdit)
                NationalID.Visible = false;
            if (IsAddOrEdit)
                str_Username.Visible = false;
            if (IsAddOrEdit)
                DrivingExperience.Visible = false;
        }
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                dshbdRegisteredList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fdshbdRegisteredsrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(str_Full_Name.AdvancedSearch.ToJson())); // Field str_Full_Name
            filters.Merge(JObject.Parse(RegistrationDate.AdvancedSearch.ToJson())); // Field Registration Date
            ...
            filters.Merge(JObject.Parse(DrivingExperience.AdvancedSearch.ToJson())); // Field Driving Experience
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fdshbdRegisteredsrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            if (filter?.TryGetValue("x_RegistrationDate", out sv) ?? false) {
                RegistrationDate.AdvancedSearch.SearchValue = sv;
                RegistrationDate.AdvancedSearch.SearchOperator = filter["z_RegistrationDate"];
                RegistrationDate.AdvancedSearch.SearchCondition = filter["v_RegistrationDate"];
                RegistrationDate.AdvancedSearch.SearchValue2 = filter["y_RegistrationDate"];
                RegistrationDate.AdvancedSearch.SearchOperator2 = filter["w_RegistrationDate"];
                RegistrationDate.AdvancedSearch.Save();
            }
            ...
            // Field Driving Experience
            if (filter?.TryGetValue("x_DrivingExperience", out sv) ?? false) {
                DrivingExperience.AdvancedSearch.SearchValue = sv;
                DrivingExperience.AdvancedSearch.SearchOperator = filter["z_DrivingExperience"];
                DrivingExperience.AdvancedSearch.SearchCondition = filter["v_DrivingExperience"];
                DrivingExperience.AdvancedSearch.SearchValue2 = filter["y_DrivingExperience"];
                DrivingExperience.AdvancedSearch.SearchOperator2 = filter["w_DrivingExperience"];
                DrivingExperience.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(DrivingExperience);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(RegistrationDate, ctrl); // Registration Date
                UpdateSort(NationalID, ctrl); // NationalID
                UpdateSort(str_Username, ctrl); // str_Username
                UpdateSort(DrivingExperience, ctrl); // Driving Experience
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdRegisteredlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdRegisteredlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fdshbdRegisteredlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fdshbdRegisteredsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fdshbdRegisteredsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdRegisteredlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_dshbdRegistered");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(dshbdRegisteredList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(dshbdRegisteredList.RowCount) + "_dshbdRegistered");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(dshbdRegisteredList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && dshbdRegisteredList.RowType == RowType.Add || IsEdit && dshbdRegisteredList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RegistrationDate.SetDbValue(row["Registration Date"]);
            NationalID.SetDbValue(row["NationalID"]);
            str_Username.SetDbValue(row["str_Username"]);
            DrivingExperience.SetDbValue(row["Driving Experience"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Registration Date", RegistrationDate.DefaultValue ?? DbNullValue); // DN
            row.Add("NationalID", NationalID.DefaultValue ?? DbNullValue); // DN
            row.Add("str_Username", str_Username.DefaultValue ?? DbNullValue); // DN
            row.Add("Driving Experience", DrivingExperience.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Registration Date
                RegistrationDate.ViewValue = RegistrationDate.CurrentValue;
                RegistrationDate.ViewValue = FormatDateTime(RegistrationDate.ViewValue, RegistrationDate.FormatPattern);
                RegistrationDate.ViewCustomAttributes = "";
                ...
                // Driving Experience
                DrivingExperience.ViewValue = ConvertToString(DrivingExperience.CurrentValue); // DN
                DrivingExperience.ViewCustomAttributes = "";
                ...
                // Registration Date
                RegistrationDate.HrefValue = "";
                RegistrationDate.TooltipValue = "";
                ...
                // Driving Experience
                DrivingExperience.HrefValue = "";
                DrivingExperience.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fdshbdRegisteredsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
