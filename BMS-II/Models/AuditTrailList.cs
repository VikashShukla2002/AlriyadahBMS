...
public partial class JUN26 {
    ...
    /// auditTrailList
    /// </summary>
    public static AuditTrailList auditTrailList
    {
        get => HttpData.Get<AuditTrailList>("auditTrailList")!;
        set => HttpData["auditTrailList"] = value;
    }

    /// <summary>
    /// Page class for AuditTrail
    /// </summary>
    public class AuditTrailList : AuditTrailListBase
    {
        // Constructor
        public AuditTrailList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public AuditTrailList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class AuditTrailListBase : AuditTrail
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "auditTrailList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fAuditTraillist";
        ...
        public AuditTrailListBase()
        {
            ...
            MultiDeleteUrl = "AuditTrailDelete";
            MultiUpdateUrl = "AuditTrailUpdate";
        ...
        // Page name
        public string PageName => "AuditTrailList";
        ...
        // Constructor
        public AuditTrailListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fAuditTrailsrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fAuditTrailsrch", filters);
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(_User);
            searchFlds.Add(_Action);
            searchFlds.Add(_Table);
            searchFlds.Add(_Field);
            searchFlds.Add(KeyValue);
            searchFlds.Add(OldValue);
            searchFlds.Add(NewValue);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(_DateTime, ctrl); // DateTime
                UpdateSort(Script, ctrl); // Script
                UpdateSort(_User, ctrl); // User
                UpdateSort(_Action, ctrl); // Action
                UpdateSort(_Table, ctrl); // Table
                UpdateSort(_Field, ctrl); // Field
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fAuditTraillist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fAuditTraillist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fAuditTraillist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fAuditTraillist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(auditTrailList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(auditTrailList.RowCount) + "_AuditTrail");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(auditTrailList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && auditTrailList.RowType == RowType.Add || IsEdit && auditTrailList.RowType == RowType.Edit) // Inline-Add/Edit row
