...
public partial class JUN26 {
    ...
    /// Page base class
    /// </summary>
    public class LoginBase : TblStudents
    {
        // Page ID
        ...
        public DbField<SqlDbType> LoginUsername = new ("tblStudents", "username", 202, SqlDbType.NVarChar) {
                TableName = "tblStudents",
                Name = "username",
                Expression = "username"
            }; // DN

        public DbField<SqlDbType> LoginPassword = new ("tblStudents", "password", 202, SqlDbType.NVarChar) {
                TableName = "tblStudents",
                Name = "password",
                Expression = "password"
            }; // DN

        public DbField<SqlDbType> LoginType = new ("tblStudents", "type", 202, SqlDbType.NVarChar) {
                TableName = "tblStudents",
                Name = "logintype",
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsLoggingIn()) { // After changing password
                ...
                if (!Empty(LoginUsername.CurrentValue)) {
                    ...
                    if (await Profile.ExceedLoginRetry(ConvertToString(LoginUsername.CurrentValue))) {
                        validate = false;
                        FailureMessage = Language.Phrase("ExceedMaxRetry").Replace("%t", ConvertToString(Config.UserProfileRetryLockout));
                    }
                }
                validPassword = false;
                if (validate) {
        ...
        {
            var usr = CurrentUser();
            await WriteAuditTrailAsync(usr, Language.Phrase("AuditTrailLogin"), CurrentUserIpAddress());
        }
