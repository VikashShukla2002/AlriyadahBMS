...
public partial class JUN26 {
    ...
    /// dshbdPayList
    /// </summary>
    public static DshbdPayList dshbdPayList
    {
        get => HttpData.Get<DshbdPayList>("dshbdPayList")!;
        set => HttpData["dshbdPayList"] = value;
    }

    /// <summary>
    /// Page class for dshbdPay
    /// </summary>
    public class DshbdPayList : DshbdPayListBase
    {
        // Constructor
        public DshbdPayList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public DshbdPayList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class DshbdPayListBase : DshbdPay
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "dshbdPay";

        // Page object name
        public string PageObjName = "dshbdPayList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fdshbdPaylist";
        ...
        public DshbdPayListBase()
        {
            ...
            // Table object (dshbdPay)
            if (dshbdPay == null || dshbdPay is DshbdPay)
                dshbdPay = this;
            ...
            MultiDeleteUrl = "DshbdPayDelete";
            MultiUpdateUrl = "DshbdPayUpdate";
        ...
        // Page name
        public string PageName => "DshbdPayList";
        ...
        // Constructor
        public DshbdPayListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "DshbdPayView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fdshbdPaysrch");
            ...
            filters.Merge(JObject.Parse(AmountPaid.AdvancedSearch.ToJson())); // Field Amount Paid
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fdshbdPaysrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Amount Paid
            if (filter?.TryGetValue("x_AmountPaid", out sv) ?? false) {
                AmountPaid.AdvancedSearch.SearchValue = sv;
                AmountPaid.AdvancedSearch.SearchOperator = filter["z_AmountPaid"];
                AmountPaid.AdvancedSearch.SearchCondition = filter["v_AmountPaid"];
                AmountPaid.AdvancedSearch.SearchValue2 = filter["y_AmountPaid"];
                AmountPaid.AdvancedSearch.SearchOperator2 = filter["w_AmountPaid"];
                AmountPaid.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(int_Payment_Method, ctrl); // int_Payment_Method
                ...
                UpdateSort(AmountPaid, ctrl); // Amount Paid
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdPaylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdPaylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fdshbdPaylist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fdshbdPaysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fdshbdPaysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdPaylist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(dshbdPayList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(dshbdPayList.RowCount) + "_dshbdPay");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(dshbdPayList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && dshbdPayList.RowType == RowType.Add || IsEdit && dshbdPayList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            AmountPaid.SetDbValue(row["Amount Paid"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Amount Paid", AmountPaid.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // int_Payment_Method
                int_Payment_Method.ViewValue = int_Payment_Method.CurrentValue;
                int_Payment_Method.ViewValue = FormatNumber(int_Payment_Method.ViewValue, int_Payment_Method.FormatPattern);
                ...
                // Amount Paid
                AmountPaid.ViewValue = ConvertToString(AmountPaid.CurrentValue); // DN
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fdshbdPaysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
