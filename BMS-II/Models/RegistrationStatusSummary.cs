...
public partial class JUN26 {
    ...
    /// </summary>
    public static RegistrationStatusSummary registrationStatusSummary
    {
        get => HttpData.Get<RegistrationStatusSummary>("registrationStatusSummary")!;
        set => HttpData["registrationStatusSummary"] = value;
    }

    /// <summary>
    /// Page class for Registration Status
    /// </summary>
    public class RegistrationStatusSummary : RegistrationStatusSummaryBase
    {
        // Constructor
        public RegistrationStatusSummary(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RegistrationStatusSummary() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class RegistrationStatusSummaryBase : RegistrationStatus
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "Registration Status";

        // Page object name
        public string PageObjName = "registrationStatusSummary";
        ...
        public RegistrationStatusSummaryBase()
        {
            ...
            // Table object (registrationStatus)
            if (registrationStatus == null || registrationStatus is RegistrationStatus)
                registrationStatus = this;
        ...
        // Page name
        public string PageName => "RegistrationStatus";
        ...
        // Constructor
        public RegistrationStatusSummaryBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run() {
            ...
            // Set up User ID
            UserIDFilter = ApplyUserIDFilters("");
            Filter = UserIDFilter;
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                registrationStatusSummary?.PageRender();
            }
        ...
        public void LoadRowValues(Dictionary<string, object> record)
        {
            ...
            data["RegistrationDate"] = record["Registration Date"];
            data["NationalID"] = record["NationalID"];
            data["DrivingExperience"] = record["Driving Experience"];
            Rows.Add(data);
            str_Username.SetDbValue(GroupValue(str_Username, record["str_Username"]));
            str_Full_Name.SetDbValue(record["str_Full_Name"]);
            RegistrationDate.SetDbValue(record["Registration Date"]);
            NationalID.SetDbValue(record["NationalID"]);
            DrivingExperience.SetDbValue(record["Driving Experience"]);
        }
        ...
        public async Task RenderRow()
        {
            if (RowType == RowType.Total && RowTotalSubType == RowTotal.Footer && RowTotalType == RowSummary.Page) { // Get Page total
                ...
                if (rsagg != null) {
                    NationalID.Count = TotalCount;
                    NationalID.CntValue = rsagg["cnt_nationalid"];
                    hasSummary = true;
                }
                ...
                if (!hasCount || !hasSummary) {
                    ...
                    NationalID.GetCnt(DetailRecords);
                }
            }
            ...
            if (RowType == RowType.Search) { // Search row
                ...
                if (RowTotalType == RowSummary.Group && RowGroupLevel >= 2)
                    RowAttrs["data-group-2"] = ConvertToString(RegistrationDate.GroupValue); // Set up group attribute 2
                if (RowTotalType == RowSummary.Group && RowGroupLevel >= 3)
                    RowAttrs["data-group-3"] = ConvertToString(DrivingExperience.GroupValue); // Set up group attribute 3
                ...
                // Registration Date
                RegistrationDate.GroupViewValue = RegistrationDate.GroupValue;
                RegistrationDate.GroupViewValue = FormatDateTime(RegistrationDate.GroupViewValue, RegistrationDate.FormatPattern);
                RegistrationDate.CellCssClass = (RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
                RegistrationDate.ViewCustomAttributes = "";
                RegistrationDate.GroupViewValue = DisplayGroupValue(RegistrationDate, RegistrationDate.GroupViewValue);
                if (!RegistrationDate.LevelBreak) {
                    if (RegistrationDate.ShowCompactSummaryFooter)
                        RegistrationDate.GroupViewValue = "&nbsp;";
                } else
                    RegistrationDate.LevelBreak = false;

                // Driving Experience
                DrivingExperience.GroupViewValue = ConvertToString(DrivingExperience.GroupValue); // DN
                DrivingExperience.CellCssClass = (RowGroupLevel == 3 ? "ew-rpt-grp-summary-3" : "ew-rpt-grp-field-3");
                DrivingExperience.ViewCustomAttributes = "";
                DrivingExperience.GroupViewValue = DisplayGroupValue(DrivingExperience, DrivingExperience.GroupViewValue);
                if (!DrivingExperience.LevelBreak) {
                    if (DrivingExperience.ShowCompactSummaryFooter)
                        DrivingExperience.GroupViewValue = "&nbsp;";
                } else
                    DrivingExperience.LevelBreak = false;

                // NationalID
                NationalID.CntViewValue = NationalID.CntValue;
                NationalID.CntViewValue = FormatNumber(NationalID.CntViewValue, NationalID.FormatPattern);
                NationalID.ViewCustomAttributes = "";
                NationalID.CellAttrs["class"] = (RowTotalType == RowSummary.Page || RowTotalType == RowSummary.Grand) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" + RowGroupLevel;
                ...
                RegistrationDate.GroupViewValue = RegistrationDate.GroupValue;
                RegistrationDate.GroupViewValue = FormatDateTime(RegistrationDate.GroupViewValue, RegistrationDate.FormatPattern);
                RegistrationDate.CellCssClass = "ew-rpt-grp-field-2";
                RegistrationDate.ViewCustomAttributes = "";
                RegistrationDate.GroupViewValue = DisplayGroupValue(RegistrationDate, RegistrationDate.GroupViewValue);
                if (!RegistrationDate.LevelBreak)
                    RegistrationDate.GroupViewValue = "&nbsp;";
                ...
                // Driving Experience
                DrivingExperience.GroupViewValue = ConvertToString(DrivingExperience.GroupValue); // DN
                DrivingExperience.CellCssClass = "ew-rpt-grp-field-3";
                DrivingExperience.ViewCustomAttributes = "";
                DrivingExperience.GroupViewValue = DisplayGroupValue(DrivingExperience, DrivingExperience.GroupViewValue);
                if (!DrivingExperience.LevelBreak)
                    DrivingExperience.GroupViewValue = "&nbsp;";
                ...
                NationalID.CellCssClass = (RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
                NationalID.ViewCustomAttributes = "";
            ...
            if (RowType == RowType.Total) {
                ...
                // Registration Date
                CellRendered(RegistrationDate, RegistrationDate.GroupViewValue, ref RegistrationDate.GroupViewValue, RegistrationDate.ViewAttrs, RegistrationDate.CellAttrs, ref RegistrationDate.HrefValue, RegistrationDate.LinkAttrs);

                // Driving Experience
                CellRendered(DrivingExperience, DrivingExperience.GroupViewValue, ref DrivingExperience.GroupViewValue, DrivingExperience.ViewAttrs, DrivingExperience.CellAttrs, ref DrivingExperience.HrefValue, DrivingExperience.LinkAttrs);

                // NationalID
                viewValue = NationalID.CntViewValue;
                CellRendered(NationalID, viewValue, ref viewValue, NationalID.ViewAttrs, NationalID.CellAttrs, ref NationalID.HrefValue, NationalID.LinkAttrs);
                NationalID.CntViewValue = ConvertToString(viewValue);
            } else {
                // str_Username
                ...
                // Registration Date
                CellRendered(RegistrationDate, RegistrationDate.CurrentValue, ref RegistrationDate.ViewValue, RegistrationDate.ViewAttrs, RegistrationDate.CellAttrs, ref RegistrationDate.HrefValue, RegistrationDate.LinkAttrs);

                // Driving Experience
                CellRendered(DrivingExperience, DrivingExperience.CurrentValue, ref DrivingExperience.ViewValue, DrivingExperience.ViewAttrs, DrivingExperience.CellAttrs, ref DrivingExperience.HrefValue, DrivingExperience.LinkAttrs);

                // NationalID
                CellRendered(NationalID, NationalID.CurrentValue, ref NationalID.ViewValue, NationalID.ViewAttrs, NationalID.CellAttrs, ref NationalID.HrefValue, NationalID.LinkAttrs);
            }
        ...
        public void SetupFieldCount()
        {
            ...
            if (str_Username.Visible)
                GroupColumnCount++;
            if (RegistrationDate.Visible) {
                GroupColumnCount++;
                SubGroupColumnCount++;
            }
            if (DrivingExperience.Visible) {
                GroupColumnCount++;
                SubGroupColumnCount++;
            }
            if (NationalID.Visible)
                DetailColumnCount++;
        }
        ...
        public void SetupOtherOptions()
        {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fRegistration_Statussrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fRegistration_Statussrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = false;
