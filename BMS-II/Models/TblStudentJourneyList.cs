...
public partial class JUN26 {
    /// <summary>
    /// tblStudentJourneyList
    /// </summary>
    public static TblStudentJourneyList tblStudentJourneyList
    {
        get => HttpData.Get<TblStudentJourneyList>("tblStudentJourneyList")!;
        set => HttpData["tblStudentJourneyList"] = value;
    }

    /// <summary>
    /// Page class for tblStudentJourney
    /// </summary>
    public class TblStudentJourneyList : TblStudentJourneyListBase
    {
        // Constructor
        public TblStudentJourneyList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblStudentJourneyList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TblStudentJourneyListBase : TblStudentJourney
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "tblStudentJourneyList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ftblStudentJourneylist";
        ...
        // Constructor
        public TblStudentJourneyListBase()
        {
            // CSS class name as context
            TableVar = "tblStudentJourney";
            ...
            MultiDeleteUrl = "TblStudentJourneyDelete";
            MultiUpdateUrl = "TblStudentJourneyUpdate";
        ...
        // Page name
        public string PageName => "TblStudentJourneyList";
        ...
        // Constructor
        public TblStudentJourneyListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "ftblStudentJourneygrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                tblStudentJourneyList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "ftblStudentJourneysrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(STDJ_ID.AdvancedSearch.ToJson())); // Field STDJ_ID
            ...
            filters.Merge(JObject.Parse(Number_ID.AdvancedSearch.ToJson())); // Field Number_ID
            filters.Merge(JObject.Parse(Journey_ID.AdvancedSearch.ToJson())); // Field Journey_ID
            filters.Merge(JObject.Parse(Journey.AdvancedSearch.ToJson())); // Field Journey
            filters.Merge(JObject.Parse(Start_Date.AdvancedSearch.ToJson())); // Field Start_Date
            filters.Merge(JObject.Parse(End_Date.AdvancedSearch.ToJson())); // Field End_Date
            filters.Merge(JObject.Parse(Total_Days.AdvancedSearch.ToJson())); // Field Total_Days
            ...
            filters.Merge(JObject.Parse(Product_Name.AdvancedSearch.ToJson())); // Field Product_Name
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "ftblStudentJourneysrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field STDJ_ID
            if (filter?.TryGetValue("x_STDJ_ID", out sv) ?? false) {
                STDJ_ID.AdvancedSearch.SearchValue = sv;
                STDJ_ID.AdvancedSearch.SearchOperator = filter["z_STDJ_ID"];
                STDJ_ID.AdvancedSearch.SearchCondition = filter["v_STDJ_ID"];
                STDJ_ID.AdvancedSearch.SearchValue2 = filter["y_STDJ_ID"];
                STDJ_ID.AdvancedSearch.SearchOperator2 = filter["w_STDJ_ID"];
                STDJ_ID.AdvancedSearch.Save();
            }
            ...
            // Field Number_ID
            if (filter?.TryGetValue("x_Number_ID", out sv) ?? false) {
                Number_ID.AdvancedSearch.SearchValue = sv;
                Number_ID.AdvancedSearch.SearchOperator = filter["z_Number_ID"];
                Number_ID.AdvancedSearch.SearchCondition = filter["v_Number_ID"];
                Number_ID.AdvancedSearch.SearchValue2 = filter["y_Number_ID"];
                Number_ID.AdvancedSearch.SearchOperator2 = filter["w_Number_ID"];
                Number_ID.AdvancedSearch.Save();
            }

            // Field Journey_ID
            if (filter?.TryGetValue("x_Journey_ID", out sv) ?? false) {
                Journey_ID.AdvancedSearch.SearchValue = sv;
                Journey_ID.AdvancedSearch.SearchOperator = filter["z_Journey_ID"];
                Journey_ID.AdvancedSearch.SearchCondition = filter["v_Journey_ID"];
                Journey_ID.AdvancedSearch.SearchValue2 = filter["y_Journey_ID"];
                Journey_ID.AdvancedSearch.SearchOperator2 = filter["w_Journey_ID"];
                Journey_ID.AdvancedSearch.Save();
            }

            // Field Journey
            if (filter?.TryGetValue("x_Journey", out sv) ?? false) {
                Journey.AdvancedSearch.SearchValue = sv;
                Journey.AdvancedSearch.SearchOperator = filter["z_Journey"];
                Journey.AdvancedSearch.SearchCondition = filter["v_Journey"];
                Journey.AdvancedSearch.SearchValue2 = filter["y_Journey"];
                Journey.AdvancedSearch.SearchOperator2 = filter["w_Journey"];
                Journey.AdvancedSearch.Save();
            }

            // Field Start_Date
            if (filter?.TryGetValue("x_Start_Date", out sv) ?? false) {
                Start_Date.AdvancedSearch.SearchValue = sv;
                Start_Date.AdvancedSearch.SearchOperator = filter["z_Start_Date"];
                Start_Date.AdvancedSearch.SearchCondition = filter["v_Start_Date"];
                Start_Date.AdvancedSearch.SearchValue2 = filter["y_Start_Date"];
                Start_Date.AdvancedSearch.SearchOperator2 = filter["w_Start_Date"];
                Start_Date.AdvancedSearch.Save();
            }

            // Field End_Date
            if (filter?.TryGetValue("x_End_Date", out sv) ?? false) {
                End_Date.AdvancedSearch.SearchValue = sv;
                End_Date.AdvancedSearch.SearchOperator = filter["z_End_Date"];
                End_Date.AdvancedSearch.SearchCondition = filter["v_End_Date"];
                End_Date.AdvancedSearch.SearchValue2 = filter["y_End_Date"];
                End_Date.AdvancedSearch.SearchOperator2 = filter["w_End_Date"];
                End_Date.AdvancedSearch.Save();
            }

            // Field Total_Days
            if (filter?.TryGetValue("x_Total_Days", out sv) ?? false) {
                Total_Days.AdvancedSearch.SearchValue = sv;
                Total_Days.AdvancedSearch.SearchOperator = filter["z_Total_Days"];
                Total_Days.AdvancedSearch.SearchCondition = filter["v_Total_Days"];
                Total_Days.AdvancedSearch.SearchValue2 = filter["y_Total_Days"];
                Total_Days.AdvancedSearch.SearchOperator2 = filter["w_Total_Days"];
                Total_Days.AdvancedSearch.Save();
            }
            ...
            // Field Product_Name
            if (filter?.TryGetValue("x_Product_Name", out sv) ?? false) {
                Product_Name.AdvancedSearch.SearchValue = sv;
                Product_Name.AdvancedSearch.SearchOperator = filter["z_Product_Name"];
                Product_Name.AdvancedSearch.SearchCondition = filter["v_Product_Name"];
                Product_Name.AdvancedSearch.SearchValue2 = filter["y_Product_Name"];
                Product_Name.AdvancedSearch.SearchOperator2 = filter["w_Product_Name"];
                Product_Name.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Journey);
            searchFlds.Add(Status);
            searchFlds.Add(Package_Name);
            searchFlds.Add(Product_Name);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(STDJ_ID, ctrl); // STDJ_ID
                UpdateSort(FullName, ctrl); // FullName
                UpdateSort(str_Username, ctrl); // str_Username
                UpdateSort(Number_ID, ctrl); // Number_ID
                UpdateSort(Journey_ID, ctrl); // Journey_ID
                UpdateSort(Journey, ctrl); // Journey
                UpdateSort(Start_Date, ctrl); // Start_Date
                UpdateSort(End_Date, ctrl); // End_Date
                UpdateSort(Total_Days, ctrl); // Total_Days
                UpdateSort(Status, ctrl); // Status
                UpdateSort(Package_Name, ctrl); // Package_Name
                UpdateSort(Product_Name, ctrl); // Product_Name
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""tblStudentJourney"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""tblStudentJourney"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""tblStudentJourney"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblStudentJourneylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblStudentJourneylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(STDJ_ID.CurrentValue) + "\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""tblStudentJourney"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ftblStudentJourneylist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ftblStudentJourneysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ftblStudentJourneysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ftblStudentJourneylist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_tblStudentJourney");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(tblStudentJourneyList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(tblStudentJourneyList.RowCount) + "_tblStudentJourney");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(tblStudentJourneyList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && tblStudentJourneyList.RowType == RowType.Add || IsEdit && tblStudentJourneyList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ftblStudentJourneysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
