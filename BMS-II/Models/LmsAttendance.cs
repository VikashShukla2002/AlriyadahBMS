...
public partial class JUN26 {
    ...
    /// </summary>
    [MaybeNull]
    public static LmsAttendance lmsAttendance
    {
        get => HttpData.Resolve<LmsAttendance>("LMS_Attendance");
        set => HttpData["LMS_Attendance"] = value;
    }

    /// <summary>
    /// Table class for LMS_Attendance
    /// </summary>
    public class LmsAttendance : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<SqlDbType> timestarted;

        public readonly DbField<SqlDbType> timecompleted;

        // Constructor
        public LmsAttendance()
        {
            // Language object // DN
            Language = ResolveLanguage();
            TableVar = "LMS_Attendance";
            Name = "LMS_Attendance";
            Type = "VIEW";
            UpdateTable = "dbo.LMS_Attendance"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            id = new (this, "x_id", 20, SqlDbType.BigInt) {
                ...
                CustomMessage = Language.FieldPhrase("LMS_Attendance", "id", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("id", id);
            ...
            userid = new (this, "x_userid", 20, SqlDbType.BigInt) {
                ...
                CustomMessage = Language.FieldPhrase("LMS_Attendance", "userid", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("userid", userid);

            // course
            course = new (this, "x_course", 20, SqlDbType.BigInt) {
                Name = "course",
                Expression = "[course]",
                BasicSearchExpression = "CAST([course] AS NVARCHAR)",
                DateTimeFormat = -1,
                ...
                DefaultErrorMessage = Language.Phrase("IncorrectInteger"),
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN" },
                CustomMessage = Language.FieldPhrase("LMS_Attendance", "course", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("course", course);

            // timestarted
            timestarted = new (this, "x_timestarted", 135, SqlDbType.DateTime) {
                Name = "timestarted",
                Expression = "[timestarted]",
                BasicSearchExpression = CastDateFieldForLike("[timestarted]", 0, "DB"),
                DateTimeFormat = 0,
                VirtualExpression = "[timestarted]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("LMS_Attendance", "timestarted", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("timestarted", timestarted);

            // timecompleted
            timecompleted = new (this, "x_timecompleted", 135, SqlDbType.DateTime) {
                Name = "timecompleted",
                Expression = "[timecompleted]",
                BasicSearchExpression = CastDateFieldForLike("[timecompleted]", 0, "DB"),
                DateTimeFormat = 0,
                VirtualExpression = "[timecompleted]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                DefaultErrorMessage = ConvertToString(Language.Phrase("IncorrectDate")).Replace("%s", CurrentDateTimeFormat.ShortDatePattern),
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("LMS_Attendance", "timecompleted", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("timecompleted", timecompleted);
        ...
        {
            get => _sqlFrom ?? "dbo.LMS_Attendance";
            set => _sqlFrom = value;
        }
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                timestarted.DbValue = row["timestarted"]; // Set DB value only
                timecompleted.DbValue = row["timecompleted"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "LmsAttendanceView"))
                return Language.Phrase("View");
            else if (SameString(pageName, "LmsAttendanceEdit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "LmsAttendanceAdd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "LmsAttendanceView",
                Config.ApiAddAction => "LmsAttendanceAdd",
                Config.ApiEditAction => "LmsAttendanceEdit",
                Config.ApiDeleteAction => "LmsAttendanceDelete",
                Config.ApiListAction => "LmsAttendanceList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "LmsAttendanceList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("LmsAttendanceView", parm);
            else
                url = KeyUrl("LmsAttendanceView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "LmsAttendanceAdd";
        ...
        public string GetAddUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = "LmsAttendanceAdd?" + parm;
            else
                url = "LmsAttendanceAdd";
        ...
        {
            string url = "";
            url = KeyUrl("LmsAttendanceEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("LmsAttendanceList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("LmsAttendanceAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("LmsAttendanceList", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("LmsAttendanceDelete")); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            ...
            timestarted.SetDbValue(dr["timestarted"]);
            timecompleted.SetDbValue(dr["timecompleted"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "LmsAttendanceList";
        ...
        public async Task RenderListRow()
        {
            ...
            // course
            course.ViewValue = course.CurrentValue;
            course.ViewValue = FormatNumber(course.ViewValue, course.FormatPattern);
            course.ViewCustomAttributes = "";

            // timestarted
            timestarted.ViewValue = timestarted.CurrentValue;
            timestarted.ViewValue = FormatDateTime(timestarted.ViewValue, timestarted.FormatPattern);
            timestarted.ViewCustomAttributes = "";

            // timecompleted
            timecompleted.ViewValue = timecompleted.CurrentValue;
            timecompleted.ViewValue = FormatDateTime(timecompleted.ViewValue, timecompleted.FormatPattern);
            timecompleted.ViewCustomAttributes = "";
            ...
            // timestarted
            timestarted.HrefValue = "";
            timestarted.TooltipValue = "";

            // timecompleted
            timecompleted.HrefValue = "";
            timecompleted.TooltipValue = "";
        ...
        public async Task RenderEditRow()
        {
            ...
            course.EditValue = course.CurrentValue; // DN
            course.PlaceHolder = RemoveHtml(course.Caption);
            if (!Empty(course.EditValue) && IsNumeric(course.EditValue))
                course.EditValue = FormatNumber(course.EditValue, null);

            // timestarted
            timestarted.SetupEditAttributes();
            timestarted.EditValue = FormatDateTime(timestarted.CurrentValue, timestarted.FormatPattern); // DN
            timestarted.PlaceHolder = RemoveHtml(timestarted.Caption);

            // timecompleted
            timecompleted.SetupEditAttributes();
            timecompleted.EditValue = FormatDateTime(timecompleted.CurrentValue, timecompleted.FormatPattern); // DN
            timecompleted.PlaceHolder = RemoveHtml(timecompleted.Caption);
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(timestarted);
                        doc.ExportCaption(timecompleted);
                    } else {
                        doc.ExportCaption(id);
                        doc.ExportCaption(userid);
                        doc.ExportCaption(course);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(timestarted);
                            await doc.ExportField(timecompleted);
                        } else {
                            await doc.ExportField(id);
                            await doc.ExportField(userid);
                            await doc.ExportField(course);
