...
public partial class JUN26 {
    ...
    /// qryInvoicesList
    /// </summary>
    public static QryInvoicesList qryInvoicesList
    {
        get => HttpData.Get<QryInvoicesList>("qryInvoicesList")!;
        set => HttpData["qryInvoicesList"] = value;
    }

    /// <summary>
    /// Page class for qryInvoices
    /// </summary>
    public class QryInvoicesList : QryInvoicesListBase
    {
        // Constructor
        public QryInvoicesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public QryInvoicesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class QryInvoicesListBase : QryInvoices
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "qryInvoices";

        // Page object name
        public string PageObjName = "qryInvoicesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fqryInvoiceslist";
        ...
        public QryInvoicesListBase()
        {
            ...
            // Table object (qryInvoices)
            if (qryInvoices == null || qryInvoices is QryInvoices)
                qryInvoices = this;

            // Initialize URLs
            AddUrl = "QryInvoicesAdd";
            ...
            MultiDeleteUrl = "QryInvoicesDelete";
            MultiUpdateUrl = "QryInvoicesUpdate";
        ...
        // Page name
        public string PageName => "QryInvoicesList";
        ...
        public void SetVisibility()
        {
            ...
            Invoice_Nbr.SetVisibility();
            InvoiceDate.SetVisibility();
            Price.SetVisibility();
        }

        // Constructor
        public QryInvoicesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "QryInvoicesView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected void HideFieldsForAddEdit() {
            ...
            if (IsAddOrEdit)
                str_ApprovalCode.Visible = false;
            if (IsAddOrEdit)
                str_Amount_Pay.Visible = false;
            if (IsAddOrEdit)
                date_Paid.Visible = false;
            if (IsAddOrEdit)
                int_Package_ID.Visible = false;
            if (IsAddOrEdit)
                Invoice_Nbr.Visible = false;
            if (IsAddOrEdit)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fqryInvoicessrch");
            ...
            filters.Merge(JObject.Parse(Invoice_Nbr.AdvancedSearch.ToJson())); // Field Invoice_Nbr
            filters.Merge(JObject.Parse(InvoiceDate.AdvancedSearch.ToJson())); // Field Invoice Date
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fqryInvoicessrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Invoice_Nbr
            if (filter?.TryGetValue("x_Invoice_Nbr", out sv) ?? false) {
                Invoice_Nbr.AdvancedSearch.SearchValue = sv;
                Invoice_Nbr.AdvancedSearch.SearchOperator = filter["z_Invoice_Nbr"];
                Invoice_Nbr.AdvancedSearch.SearchCondition = filter["v_Invoice_Nbr"];
                Invoice_Nbr.AdvancedSearch.SearchValue2 = filter["y_Invoice_Nbr"];
                Invoice_Nbr.AdvancedSearch.SearchOperator2 = filter["w_Invoice_Nbr"];
                Invoice_Nbr.AdvancedSearch.Save();
            }

            // Field Invoice Date
            if (filter?.TryGetValue("x_InvoiceDate", out sv) ?? false) {
                InvoiceDate.AdvancedSearch.SearchValue = sv;
                InvoiceDate.AdvancedSearch.SearchOperator = filter["z_InvoiceDate"];
                InvoiceDate.AdvancedSearch.SearchCondition = filter["v_InvoiceDate"];
                InvoiceDate.AdvancedSearch.SearchValue2 = filter["y_InvoiceDate"];
                InvoiceDate.AdvancedSearch.SearchOperator2 = filter["w_InvoiceDate"];
                InvoiceDate.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Invoice_Nbr);
            searchFlds.Add(InvoiceDate);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Course_Price, ctrl); // Course_Price
                UpdateSort(Vat_Tax_15, ctrl); // Vat_Tax_15
                UpdateSort(Total_Price, ctrl); // Total_Price
                ...
                UpdateSort(Invoice_Nbr, ctrl); // Invoice_Nbr
                UpdateSort(InvoiceDate, ctrl); // Invoice Date
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryInvoiceslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryInvoiceslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fqryInvoiceslist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fqryInvoicessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fqryInvoicessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fqryInvoiceslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_qryInvoices");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(qryInvoicesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(qryInvoicesList.RowCount) + "_qryInvoices");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(qryInvoicesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && qryInvoicesList.RowType == RowType.Add || IsEdit && qryInvoicesList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Invoice_Nbr.SetDbValue(row["Invoice_Nbr"]);
            InvoiceDate.SetDbValue(row["Invoice Date"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Invoice_Nbr", Invoice_Nbr.DefaultValue ?? DbNullValue); // DN
            row.Add("Invoice Date", InvoiceDate.DefaultValue ?? DbNullValue); // DN
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fqryInvoicessrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
