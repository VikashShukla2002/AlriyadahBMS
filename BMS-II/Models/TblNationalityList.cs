...
public partial class JUN26 {
    ...
    /// tblNationalityList
    /// </summary>
    public static TblNationalityList tblNationalityList
    {
        get => HttpData.Get<TblNationalityList>("tblNationalityList")!;
        set => HttpData["tblNationalityList"] = value;
    }

    /// <summary>
    /// Page class for tblNationality
    /// </summary>
    public class TblNationalityList : TblNationalityListBase
    {
        // Constructor
        public TblNationalityList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblNationalityList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TblNationalityListBase : TblNationality
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "tblNationalityList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ftblNationalitylist";
        ...
        // Constructor
        public TblNationalityListBase()
        {
            // CSS class name as context
            TableVar = "tblNationality";
            ...
            MultiDeleteUrl = "TblNationalityDelete";
            MultiUpdateUrl = "TblNationalityUpdate";
        ...
        // Page name
        public string PageName => "TblNationalityList";
        ...
        // Constructor
        public TblNationalityListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "ftblNationalitysrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(ID.AdvancedSearch.ToJson())); // Field ID
            filters.Merge(JObject.Parse(Nationality_en.AdvancedSearch.ToJson())); // Field Nationality_en
            filters.Merge(JObject.Parse(Nationality_arabic.AdvancedSearch.ToJson())); // Field Nationality_arabic
            filters.Merge(JObject.Parse(IsActive.AdvancedSearch.ToJson())); // Field IsActive
            filters.Merge(JObject.Parse(IsDeleted.AdvancedSearch.ToJson())); // Field IsDeleted
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "ftblNationalitysrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Nationality_en
            if (filter?.TryGetValue("x_Nationality_en", out sv) ?? false) {
                Nationality_en.AdvancedSearch.SearchValue = sv;
                Nationality_en.AdvancedSearch.SearchOperator = filter["z_Nationality_en"];
                Nationality_en.AdvancedSearch.SearchCondition = filter["v_Nationality_en"];
                Nationality_en.AdvancedSearch.SearchValue2 = filter["y_Nationality_en"];
                Nationality_en.AdvancedSearch.SearchOperator2 = filter["w_Nationality_en"];
                Nationality_en.AdvancedSearch.Save();
            }

            // Field Nationality_arabic
            if (filter?.TryGetValue("x_Nationality_arabic", out sv) ?? false) {
                Nationality_arabic.AdvancedSearch.SearchValue = sv;
                Nationality_arabic.AdvancedSearch.SearchOperator = filter["z_Nationality_arabic"];
                Nationality_arabic.AdvancedSearch.SearchCondition = filter["v_Nationality_arabic"];
                Nationality_arabic.AdvancedSearch.SearchValue2 = filter["y_Nationality_arabic"];
                Nationality_arabic.AdvancedSearch.SearchOperator2 = filter["w_Nationality_arabic"];
                Nationality_arabic.AdvancedSearch.Save();
            }
            ...
            // Field IsDeleted
            if (filter?.TryGetValue("x_IsDeleted", out sv) ?? false) {
                IsDeleted.AdvancedSearch.SearchValue = sv;
                IsDeleted.AdvancedSearch.SearchOperator = filter["z_IsDeleted"];
                IsDeleted.AdvancedSearch.SearchCondition = filter["v_IsDeleted"];
                IsDeleted.AdvancedSearch.SearchValue2 = filter["y_IsDeleted"];
                IsDeleted.AdvancedSearch.SearchOperator2 = filter["w_IsDeleted"];
                IsDeleted.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Nationality_en);
            searchFlds.Add(Nationality_arabic);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Nationality_en, ctrl); // Nationality_en
                UpdateSort(Nationality_arabic, ctrl); // Nationality_arabic
                UpdateSort(IsActive, ctrl); // IsActive
                UpdateSort(IsDeleted, ctrl); // IsDeleted
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblNationalitylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblNationalitylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""tblNationality"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ftblNationalitylist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ftblNationalitysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ftblNationalitysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ftblNationalitylist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_tblNationality");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(tblNationalityList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(tblNationalityList.RowCount) + "_tblNationality");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(tblNationalityList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && tblNationalityList.RowType == RowType.Add || IsEdit && tblNationalityList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                Nationality_en.TooltipValue = "";

                // Nationality_arabic
                Nationality_arabic.HrefValue = "";
                Nationality_arabic.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ftblNationalitysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
