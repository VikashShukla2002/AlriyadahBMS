        ...
        where T : DbField
    {
        ...
        public string GetSql(bool useParentFilter = true, string currentFilter = "", Func<string>? lookupFilter = null, dynamic? page = null, bool skipFilterFields = false, bool clearUserFilter = false)
        {
            ...
            if (page != null) {
                ...
                if (page.Fields.TryGetValue(Field?.Name ?? "", out T fld))
                    Invoke(page, "LookupSelecting", new object[] { (DbField)fld, newFilter }); // Call Lookup Selecting
            ...
            if (row is Dictionary<string, object> dict) {
                ...
                for (int i = 1; i < row.Count; i++) {
                    ...
                    if (!Empty(name) && renderer.Fields.ContainsKey(name)) {
                        ...
                        if (mi != null && !Empty(field.ViewValue) && !(sameTable && SameString(name, Field?.Name))) {
            ...
            // Handle List
                ...
                for (int i = 1; i < row.Count; i++) {
                    ...
                    if (!Empty(name) && renderer.Fields.ContainsKey(name)) {
                        ...
                        if (mi != null && !Empty(field.ViewValue) && !(sameTable && SameString(name, Field?.Name) && !(new List<DataType> { DataType.Date, DataType.Time }).Contains(field.DataType) && field.OptionCount == 0))
        ...
        protected string GetFilter(T fld, string opr, string val, string dbid)
        {
            ...
            string[] values = Field?.IsMultiSelect ?? false ? val.Split(Config.MultipleOptionSeparator) : new string[] { val };
        ...
        public virtual dynamic? GetTable(bool create = false)
        {
            ...
            if (create && Table != null)  { // Create new instance
                if (Config.NamedTypes.TryGetValue(Table!.TableVar, out Type? classType) && classType != null)
                    return CreateInstance(classType!.Name);
            }
            return Table;
        }
