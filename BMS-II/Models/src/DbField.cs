...
public partial class JUN26 {
    ...
    public class DbField
    {
        ...
        public DataType DataType; // Field type (ASP.NET Maker data type)
        ...
        public OptionValues LookupCacheOption(string? val)
        {
            ...
            if (Empty(val) || Empty(Lookup) || Lookup.Options.Count == 0)
                return res;
            if (IsMultiSelect) { // Multiple options
                var arwrk = val.Split(Config.MultipleOptionSeparator);
        ...
        public virtual IHtmlContent SelectOptionListHtml(string name = "", bool? multiple = null)
        {
            ...
            string?[] curValues = isMultiple && !Empty(curValue) ? ConvertToString(curValue).Split(Config.MultipleOptionSeparator) : new string?[] {};
