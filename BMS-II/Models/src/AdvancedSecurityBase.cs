...
public partial class JUN26 {
    ...
    public class AdvancedSecurityBase
    {
        ...
        public void LoadUserID()
        {
            if (Empty(CurrentUserID)) {
                ...
                while (userIdList != currentUserIdList) {
                    filter = MultiValueFilter("[Parent_Username]", UserID, Config.UserTableDbId);
        ...
        public string UserIDList() =>
            String.Join(", ", UserID.Select(id => QuotedValue(id, DataType.String, Config.UserTableDbId)));
        ...
        public string ParentUserIDList(object? userid)
        {
            if (SameString(userid, CurrentUserID)) // Own record
                return String.Join(", ", ParentUserID.Select(id => QuotedValue(id, DataType.String, Config.UserTableDbId)));
            else
                return String.Join(", ", UserID.Where(id => !SameString(id, userid)).Select(id => QuotedValue(id, DataType.String, Config.UserTableDbId)));
        }
