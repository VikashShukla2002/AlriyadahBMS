...
public partial class JUN26 {
    ...
    /// </summary>
    [MaybeNull]
    public static HijriTable hijriTable
    {
        get => HttpData.Resolve<HijriTable>("Hijri_Table");
        set => HttpData["Hijri_Table"] = value;
    }

    /// <summary>
    /// Table class for Hijri_Table
    /// </summary>
    public class HijriTable : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<SqlDbType> Hijri_Monthname;

        public readonly DbField<SqlDbType> Hijri_Day;

        public readonly DbField<SqlDbType> Hijri_mon_ar;
        ...
        public HijriTable()
        {
            ...
            Name = "Hijri_Table";
            Type = "TABLE";
            UpdateTable = "dbo.Hijri_Table"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            ID = new (this, "x_ID", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("Hijri_Table", "ID", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("ID", ID);
            ...
            Hijri_Year = new (this, "x_Hijri_Year", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("Hijri_Table", "Hijri_Year", "CustomMsg"),
                IsUpload = false
            };
            ...
            Hijri_Month = new (this, "x_Hijri_Month", 3, SqlDbType.Int) {
                ...
                DefaultErrorMessage = Language.Phrase("IncorrectInteger"),
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("Hijri_Table", "Hijri_Month", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Hijri_Month", Hijri_Month);

            // Hijri_Monthname
            Hijri_Monthname = new (this, "x_Hijri_Monthname", 200, SqlDbType.VarChar) {
                Name = "Hijri_Monthname",
                Expression = "[Hijri_Monthname]",
                UseBasicSearch = true,
                BasicSearchExpression = "[Hijri_Monthname]",
                DateTimeFormat = -1,
                VirtualExpression = "[Hijri_Monthname]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("Hijri_Table", "Hijri_Monthname", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Hijri_Monthname", Hijri_Monthname);
            ...
            Hijri_Day = new (this, "x_Hijri_Day", 3, SqlDbType.Int) {
                ...
                DefaultErrorMessage = Language.Phrase("IncorrectInteger"),
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("Hijri_Table", "Hijri_Day", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Hijri_Day", Hijri_Day);

            // Hijri_mon_ar
            Hijri_mon_ar = new (this, "x_Hijri_mon_ar", 202, SqlDbType.NVarChar) {
                Name = "Hijri_mon_ar",
                Expression = "[Hijri_mon_ar]",
                UseBasicSearch = true,
                BasicSearchExpression = "[Hijri_mon_ar]",
                DateTimeFormat = -1,
                VirtualExpression = "[Hijri_mon_ar]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                InputTextType = "text",
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("Hijri_Table", "Hijri_mon_ar", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Hijri_mon_ar", Hijri_mon_ar);
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                Hijri_Monthname.DbValue = row["Hijri_Monthname"]; // Set DB value only
                Hijri_Day.DbValue = row["Hijri_Day"]; // Set DB value only
                Hijri_mon_ar.DbValue = row["Hijri_mon_ar"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "HijriTableView"))
                return Language.Phrase("View");
            else if (SameString(pageName, "HijriTableEdit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "HijriTableAdd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "HijriTableView",
                Config.ApiAddAction => "HijriTableAdd",
                Config.ApiEditAction => "HijriTableEdit",
                Config.ApiDeleteAction => "HijriTableDelete",
                Config.ApiListAction => "HijriTableList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "HijriTableList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("HijriTableView", parm);
            else
                url = KeyUrl("HijriTableView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "HijriTableAdd";
        ...
        {
            string url = "";
            url = KeyUrl("HijriTableEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("HijriTableList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("HijriTableAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("HijriTableList", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("HijriTableDelete")); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            ...
            Hijri_Monthname.SetDbValue(dr["Hijri_Monthname"]);
            Hijri_Day.SetDbValue(dr["Hijri_Day"]);
            Hijri_mon_ar.SetDbValue(dr["Hijri_mon_ar"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "HijriTableList";
        ...
        public async Task RenderEditRow()
        {
            ...
            // Hijri_Monthname
            Hijri_Monthname.SetupEditAttributes();
            if (!Hijri_Monthname.Raw)
                Hijri_Monthname.CurrentValue = HtmlDecode(Hijri_Monthname.CurrentValue);
            Hijri_Monthname.EditValue = HtmlEncode(Hijri_Monthname.CurrentValue);
            Hijri_Monthname.PlaceHolder = RemoveHtml(Hijri_Monthname.Caption);
            ...
            // Hijri_mon_ar
            Hijri_mon_ar.SetupEditAttributes();
            if (!Hijri_mon_ar.Raw)
                Hijri_mon_ar.CurrentValue = HtmlDecode(Hijri_mon_ar.CurrentValue);
            Hijri_mon_ar.EditValue = HtmlEncode(Hijri_mon_ar.CurrentValue);
            Hijri_mon_ar.PlaceHolder = RemoveHtml(Hijri_mon_ar.Caption);
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(Hijri_Monthname);
                        doc.ExportCaption(Hijri_Day);
                        doc.ExportCaption(Hijri_mon_ar);
                    } else {
                        doc.ExportCaption(ID);
                        doc.ExportCaption(Hijri_Year);
                        doc.ExportCaption(Hijri_Month);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(Hijri_Monthname);
                            await doc.ExportField(Hijri_Day);
                            await doc.ExportField(Hijri_mon_ar);
                        } else {
                            await doc.ExportField(ID);
                            await doc.ExportField(Hijri_Year);
                            await doc.ExportField(Hijri_Month);
