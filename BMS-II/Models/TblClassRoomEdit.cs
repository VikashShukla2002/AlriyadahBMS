...
public partial class JUN26 {
    ...
    /// tblClassRoomEdit
    /// </summary>
    public static TblClassRoomEdit tblClassRoomEdit
    {
        get => HttpData.Get<TblClassRoomEdit>("tblClassRoomEdit")!;
        set => HttpData["tblClassRoomEdit"] = value;
    }

    /// <summary>
    /// Page class for tblClassRoom
    /// </summary>
    public class TblClassRoomEdit : TblClassRoomEditBase
    {
        // Constructor
        public TblClassRoomEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblClassRoomEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TblClassRoomEditBase : TblClassRoom
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "tblClassRoomEdit";
        ...
        // Page name
        public string PageName => "TblClassRoomEdit";
        ...
        // Constructor
        public TblClassRoomEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("int_CR_ID", out rv)) { // DN
                    int_CR_ID.FormValue = UrlDecode(rv); // DN
                    int_CR_ID.OldValue = int_CR_ID.FormValue;
                } else if (CurrentForm.HasValue("x_int_CR_ID")) {
                    int_CR_ID.FormValue = CurrentForm.GetValue("x_int_CR_ID");
                    int_CR_ID.OldValue = int_CR_ID.FormValue;
                } else if (!Empty(keyValues)) {
                    int_CR_ID.OldValue = ConvertToString(keyValues[0]);
                } else {
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("int_CR_ID", out rv)) { // DN
                        ...
                        int_CR_ID.CurrentValue = DbNullValue;
                    }
                }
        ...
        if (postBack) {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                int_CR_ID.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("TblClassRoomList"); // No matching record, return to list
                        }
                    break;
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                curVal = ConvertToString(str_Package_Name.CurrentValue)?.Trim() ?? "";
                if (str_Package_Name.Lookup != null && IsDictionary(str_Package_Name.Lookup?.Options) && str_Package_Name.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    str_Package_Name.EditValue = str_Package_Name.Lookup?.Options.Values.ToList();
                } else { // Lookup from database
                    if (curVal == "") {
                        filterWrk = "0=1";
                    } else {
                    ...
                    sqlWrk = str_Package_Name.Lookup?.GetSql(true, filterWrk, null, this, false, true);
                    rswrk = sqlWrk != null ? Connection.GetRows(sqlWrk) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                    str_Package_Name.EditValue = rswrk;
                }
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // str_Package_Name
            str_Package_Name.SetDbValue(rsnew, str_Package_Name.CurrentValue, str_Package_Name.ReadOnly);
            ...
            // int_MU_Size
            int_MU_Size.SetDbValue(rsnew, int_MU_Size.CurrentValue, int_MU_Size.ReadOnly);

            // int_CR_Status
            int_CR_Status.SetDbValue(rsnew, int_CR_Status.CurrentValue, int_CR_Status.ReadOnly);

            // Is_Web_SignUp
            Is_Web_SignUp.SetDbValue(rsnew, ConvertToBool(Is_Web_SignUp.CurrentValue), Is_Web_SignUp.ReadOnly);

            // int_TotSession
            int_TotSession.SetDbValue(rsnew, int_TotSession.CurrentValue, int_TotSession.ReadOnly);

            // int_PerDaySession
            int_PerDaySession.SetDbValue(rsnew, int_PerDaySession.CurrentValue, int_PerDaySession.ReadOnly);
            ...
            // str_CR_Notes
            str_CR_Notes.SetDbValue(rsnew, str_CR_Notes.CurrentValue, str_CR_Notes.ReadOnly);

            // is_ZoomMeet
            is_ZoomMeet.SetDbValue(rsnew, ConvertToBool(is_ZoomMeet.CurrentValue), is_ZoomMeet.ReadOnly);

            // BulkCR_Set
            BulkCR_Set.SetDbValue(rsnew, BulkCR_Set.CurrentValue, BulkCR_Set.ReadOnly);
            ...
            // int_Day_Incremental
            int_Day_Incremental.SetDbValue(rsnew, int_Day_Incremental.CurrentValue, int_Day_Incremental.ReadOnly);

            // int_Reoccurrence
            int_Reoccurrence.SetDbValue(rsnew, int_Reoccurrence.CurrentValue, int_Reoccurrence.ReadOnly);

            // date_Start2
            date_Start2.SetDbValue(rsnew, ConvertToDateTime(date_Start2.CurrentValue, date_Start2.FormatPattern), date_Start2.ReadOnly);

            // date_Start3
            date_Start3.SetDbValue(rsnew, ConvertToDateTime(date_Start3.CurrentValue, date_Start3.FormatPattern), date_Start3.ReadOnly);

            // date_Start4
            date_Start4.SetDbValue(rsnew, ConvertToDateTime(date_Start4.CurrentValue, date_Start4.FormatPattern), date_Start4.ReadOnly);
