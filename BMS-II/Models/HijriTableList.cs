...
public partial class JUN26 {
    ...
    /// hijriTableList
    /// </summary>
    public static HijriTableList hijriTableList
    {
        get => HttpData.Get<HijriTableList>("hijriTableList")!;
        set => HttpData["hijriTableList"] = value;
    }

    /// <summary>
    /// Page class for Hijri_Table
    /// </summary>
    public class HijriTableList : HijriTableListBase
    {
        // Constructor
        public HijriTableList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public HijriTableList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class HijriTableListBase : HijriTable
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "Hijri_Table";

        // Page object name
        public string PageObjName = "hijriTableList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fHijri_Tablelist";
        ...
        // Constructor
        public HijriTableListBase()
        {
            // CSS class name as context
            TableVar = "Hijri_Table";
            ...
            // Table object (hijriTable)
            if (hijriTable == null || hijriTable is HijriTable)
                hijriTable = this;

            // Initialize URLs
            AddUrl = "HijriTableAdd";
            ...
            MultiDeleteUrl = "HijriTableDelete";
            MultiUpdateUrl = "HijriTableUpdate";
        ...
        // Page name
        public string PageName => "HijriTableList";
        ...
        public void SetVisibility()
        {
            ...
            Hijri_Monthname.SetVisibility();
            Hijri_Day.SetVisibility();
            Hijri_mon_ar.SetVisibility();
        }

        // Constructor
        public HijriTableListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "HijriTableView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fHijri_Tablesrch");
            ...
            filters.Merge(JObject.Parse(Hijri_Monthname.AdvancedSearch.ToJson())); // Field Hijri_Monthname
            filters.Merge(JObject.Parse(Hijri_Day.AdvancedSearch.ToJson())); // Field Hijri_Day
            filters.Merge(JObject.Parse(Hijri_mon_ar.AdvancedSearch.ToJson())); // Field Hijri_mon_ar
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fHijri_Tablesrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Hijri_Monthname
            if (filter?.TryGetValue("x_Hijri_Monthname", out sv) ?? false) {
                Hijri_Monthname.AdvancedSearch.SearchValue = sv;
                Hijri_Monthname.AdvancedSearch.SearchOperator = filter["z_Hijri_Monthname"];
                Hijri_Monthname.AdvancedSearch.SearchCondition = filter["v_Hijri_Monthname"];
                Hijri_Monthname.AdvancedSearch.SearchValue2 = filter["y_Hijri_Monthname"];
                Hijri_Monthname.AdvancedSearch.SearchOperator2 = filter["w_Hijri_Monthname"];
                Hijri_Monthname.AdvancedSearch.Save();
            }
            ...
            // Field Hijri_mon_ar
            if (filter?.TryGetValue("x_Hijri_mon_ar", out sv) ?? false) {
                Hijri_mon_ar.AdvancedSearch.SearchValue = sv;
                Hijri_mon_ar.AdvancedSearch.SearchOperator = filter["z_Hijri_mon_ar"];
                Hijri_mon_ar.AdvancedSearch.SearchCondition = filter["v_Hijri_mon_ar"];
                Hijri_mon_ar.AdvancedSearch.SearchValue2 = filter["y_Hijri_mon_ar"];
                Hijri_mon_ar.AdvancedSearch.SearchOperator2 = filter["w_Hijri_mon_ar"];
                Hijri_mon_ar.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Hijri_Monthname);
            searchFlds.Add(Hijri_mon_ar);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Hijri_Monthname, ctrl); // Hijri_Monthname
                UpdateSort(Hijri_Day, ctrl); // Hijri_Day
                UpdateSort(Hijri_mon_ar, ctrl); // Hijri_mon_ar
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fHijri_Tablelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fHijri_Tablelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fHijri_Tablelist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fHijri_Tablesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fHijri_Tablesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fHijri_Tablelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_Hijri_Table");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(hijriTableList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(hijriTableList.RowCount) + "_Hijri_Table");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(hijriTableList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && hijriTableList.RowType == RowType.Add || IsEdit && hijriTableList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Hijri_Monthname.SetDbValue(row["Hijri_Monthname"]);
            Hijri_Day.SetDbValue(row["Hijri_Day"]);
            Hijri_mon_ar.SetDbValue(row["Hijri_mon_ar"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Hijri_Monthname", Hijri_Monthname.DefaultValue ?? DbNullValue); // DN
            row.Add("Hijri_Day", Hijri_Day.DefaultValue ?? DbNullValue); // DN
            row.Add("Hijri_mon_ar", Hijri_mon_ar.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Hijri_Monthname
                Hijri_Monthname.ViewValue = ConvertToString(Hijri_Monthname.CurrentValue); // DN
                Hijri_Monthname.ViewCustomAttributes = "";
                ...
                // Hijri_mon_ar
                Hijri_mon_ar.ViewValue = ConvertToString(Hijri_mon_ar.CurrentValue); // DN
                Hijri_mon_ar.ViewCustomAttributes = "";
                ...
                // Hijri_Monthname
                Hijri_Monthname.HrefValue = "";
                Hijri_Monthname.TooltipValue = "";
                ...
                // Hijri_mon_ar
                Hijri_mon_ar.HrefValue = "";
                Hijri_mon_ar.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fHijri_Tablesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
