...
public partial class JUN26 {
    /// <summary>
    /// appointmentCalendarCalendar
    /// </summary>
    public static AppointmentCalendarCalendar appointmentCalendarCalendar
    {
        get => HttpData.Get<AppointmentCalendarCalendar>("appointmentCalendarCalendar")!;
        set => HttpData["appointmentCalendarCalendar"] = value;
    }

    /// <summary>
    /// Page class for Appointment_Calendar
    /// </summary>
    public class AppointmentCalendarCalendar : AppointmentCalendarCalendarBase
    {
        // Constructor
        public AppointmentCalendarCalendar(Controller controller) : base(controller)
        {
        }

        // Constructor
        public AppointmentCalendarCalendar() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class AppointmentCalendarCalendarBase : AppointmentCalendar
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "appointmentCalendarCalendar";
        ...
        // Constructor
        public AppointmentCalendarCalendarBase()
        {
            // Initialize
        ...
        // Page name
        public string PageName => "AppointmentCalendar";
        ...
        // Constructor
        public AppointmentCalendarCalendarBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            EventViewUrl = Security.CanView ? "AppointmentCalendarView" : "";
            EventAddUrl = Security.CanAdd ? "AppointmentCalendarAdd" : "";
            EventEditUrl = Security.CanEdit ? "AppointmentCalendarEdit" : "";
            EventCopyUrl = Security.CanAdd ? "AppointmentCalendarAdd" : "";
            EventDeleteUrl = Security.CanDelete ? "AppointmentCalendarDelete" : "";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                appointmentCalendarCalendar?.PageRender();
            }
            return PageResult();
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // int_Enrollement_Id
                curVal = SameString(int_Enrollement_Id.CurrentValue, Config.InitValue) ? "" : ConvertToString(int_Enrollement_Id.CurrentValue)?.Trim() ?? "";
                if (!Empty(curVal)) {
                ...
                // int_PackageID
                curVal = SameString(int_PackageID.CurrentValue, Config.InitValue) ? "" : ConvertToString(int_PackageID.CurrentValue)?.Trim() ?? "";
                if (!Empty(curVal)) {
                ...
                // Title
                curVal = SameString(_Title.CurrentValue, Config.InitValue) ? "" : ConvertToString(_Title.CurrentValue)?.Trim() ?? "";
                if (!Empty(curVal)) {
                    if (_Title.Lookup != null && IsDictionary(_Title.Lookup?.Options) && _Title.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        filterWrk = SearchFilter("[Title]", "=", _Title.CurrentValue, DataType.String, "");
                ...
                // str_AppointmentType
                curVal = SameString(str_AppointmentType.CurrentValue, Config.InitValue) ? "" : ConvertToString(str_AppointmentType.CurrentValue)?.Trim() ?? "";
                if (!Empty(curVal)) {
                    if (str_AppointmentType.Lookup != null && IsDictionary(str_AppointmentType.Lookup?.Options) && str_AppointmentType.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        ...
                        filterWrk = SearchFilter("[Journey]", "=", str_AppointmentType.CurrentValue, DataType.String, "");
        ...
        {
            List<string> eventListFields = new () { "int_Enrollement_Id", "int_PackageID", "Title", "Start", "End", "AllDay", "Description", "Url", "Display", "BackgroundColor", "CRSS_ID", "str_AppointmentType", "str_AppointmentStatus", "str_Username", "str_CRSS_IDUSER" };
        ...
        public void SetupOtherOptions()
        {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fAppointment_Calendarsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fAppointment_Calendarsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = false;
