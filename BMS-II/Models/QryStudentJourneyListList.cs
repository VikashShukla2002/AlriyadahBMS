...
public partial class JUN26 {
    /// <summary>
    /// qryStudentJourneyListList
    /// </summary>
    public static QryStudentJourneyListList qryStudentJourneyListList
    {
        get => HttpData.Get<QryStudentJourneyListList>("qryStudentJourneyListList")!;
        set => HttpData["qryStudentJourneyListList"] = value;
    }

    /// <summary>
    /// Page class for qryStudentJourneyList
    /// </summary>
    public class QryStudentJourneyListList : QryStudentJourneyListListBase
    {
        // Constructor
        public QryStudentJourneyListList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public QryStudentJourneyListList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class QryStudentJourneyListListBase : QryStudentJourneyList
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "qryStudentJourneyList";

        // Page object name
        public string PageObjName = "qryStudentJourneyListList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fqryStudentJourneyListlist";
        ...
        // Constructor
        public QryStudentJourneyListListBase()
        {
            // CSS class name as context
            TableVar = "qryStudentJourneyList";
            ...
            // Table object (qryStudentJourneyList)
            if (qryStudentJourneyList == null || qryStudentJourneyList is QryStudentJourneyList)
                qryStudentJourneyList = this;

            // Initialize URLs
            AddUrl = "QryStudentJourneyListAdd";
            ...
            MultiDeleteUrl = "QryStudentJourneyListDelete";
            MultiUpdateUrl = "QryStudentJourneyListUpdate";
        ...
        // Page name
        public string PageName => "QryStudentJourneyListList";
        ...
        {
            Package_No.SetVisibility();
            Package_Name.SetVisibility();
            Item_No.SetVisibility();
            Service_Description.SetVisibility();
            Price.SetVisibility();
            Journey.SetVisibility();
        }

        // Constructor
        public QryStudentJourneyListListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "QryStudentJourneyListView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected void HideFieldsForAddEdit() {
            if (IsAddOrEdit)
                Package_No.Visible = false;
            if (IsAddOrEdit)
                Package_Name.Visible = false;
            if (IsAddOrEdit)
                Item_No.Visible = false;
            if (IsAddOrEdit)
                Price.Visible = false;
            if (IsAddOrEdit)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fqryStudentJourneyListgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                qryStudentJourneyListList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fqryStudentJourneyListsrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(Package_No.AdvancedSearch.ToJson())); // Field Package_No#
            ...
            filters.Merge(JObject.Parse(Service_Description.AdvancedSearch.ToJson())); // Field Service_Description
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fqryStudentJourneyListsrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Package_No#
            if (filter?.TryGetValue("x_Package_No", out sv) ?? false) {
                Package_No.AdvancedSearch.SearchValue = sv;
                Package_No.AdvancedSearch.SearchOperator = filter["z_Package_No"];
                Package_No.AdvancedSearch.SearchCondition = filter["v_Package_No"];
                Package_No.AdvancedSearch.SearchValue2 = filter["y_Package_No"];
                Package_No.AdvancedSearch.SearchOperator2 = filter["w_Package_No"];
                Package_No.AdvancedSearch.Save();
            }
            ...
            if (filter?.TryGetValue("x_Service_Description", out sv) ?? false) {
                Service_Description.AdvancedSearch.SearchValue = sv;
                Service_Description.AdvancedSearch.SearchOperator = filter["z_Service_Description"];
                Service_Description.AdvancedSearch.SearchCondition = filter["v_Service_Description"];
                Service_Description.AdvancedSearch.SearchValue2 = filter["y_Service_Description"];
                Service_Description.AdvancedSearch.SearchOperator2 = filter["w_Service_Description"];
                Service_Description.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Service_Description);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Package_No, ctrl); // Package_No#
                UpdateSort(Package_Name, ctrl); // Package_Name
                UpdateSort(Item_No, ctrl); // Item_No#
                UpdateSort(Service_Description, ctrl); // Service_Description
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryStudentJourneyListlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryStudentJourneyListlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fqryStudentJourneyListlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fqryStudentJourneyListsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fqryStudentJourneyListsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fqryStudentJourneyListlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_qryStudentJourneyList");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(qryStudentJourneyListList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(qryStudentJourneyListList.RowCount) + "_qryStudentJourneyList");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(qryStudentJourneyListList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && qryStudentJourneyListList.RowType == RowType.Add || IsEdit && qryStudentJourneyListList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RowSelected(row);
            Package_No.SetDbValue(row["Package_No#"]);
            Package_Name.SetDbValue(row["Package_Name"]);
            Item_No.SetDbValue(row["Item_No#"]);
            Service_Description.SetDbValue(row["Service_Description"]);
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("Package_No#", Package_No.DefaultValue ?? DbNullValue); // DN
            row.Add("Package_Name", Package_Name.DefaultValue ?? DbNullValue); // DN
            row.Add("Item_No#", Item_No.DefaultValue ?? DbNullValue); // DN
            row.Add("Service_Description", Service_Description.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // Package_No#
                Package_No.ViewValue = Package_No.CurrentValue;
                Package_No.ViewValue = FormatNumber(Package_No.ViewValue, Package_No.FormatPattern);
                Package_No.ViewCustomAttributes = "";
                ...
                Item_No.ViewValue = FormatNumber(Item_No.ViewValue, Item_No.FormatPattern);
                Item_No.ViewCustomAttributes = "";

                // Service_Description
                Service_Description.ViewValue = ConvertToString(Service_Description.CurrentValue); // DN
                Service_Description.ViewCustomAttributes = "";
                ...
                // Package_No#
                Package_No.HrefValue = "";
                Package_No.TooltipValue = "";
                ...
                // Service_Description
                Service_Description.HrefValue = "";
                Service_Description.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fqryStudentJourneyListsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
