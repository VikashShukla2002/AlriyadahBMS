...
public partial class JUN26 {
    /// <summary>
    /// tblOtherWebServicesList
    /// </summary>
    public static TblOtherWebServicesList tblOtherWebServicesList
    {
        get => HttpData.Get<TblOtherWebServicesList>("tblOtherWebServicesList")!;
        set => HttpData["tblOtherWebServicesList"] = value;
    }

    /// <summary>
    /// Page class for tblOtherWebServices
    /// </summary>
    public class TblOtherWebServicesList : TblOtherWebServicesListBase
    {
        // Constructor
        public TblOtherWebServicesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblOtherWebServicesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TblOtherWebServicesListBase : TblOtherWebServices
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "tblOtherWebServicesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ftblOtherWebServiceslist";
        ...
        // Constructor
        public TblOtherWebServicesListBase()
        {
            // CSS class name as context
            TableVar = "tblOtherWebServices";
            ...
            MultiDeleteUrl = "TblOtherWebServicesDelete";
            MultiUpdateUrl = "TblOtherWebServicesUpdate";
        ...
        // Page name
        public string PageName => "TblOtherWebServicesList";
        ...
        // Constructor
        public TblOtherWebServicesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "ftblOtherWebServicesgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                tblOtherWebServicesList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "ftblOtherWebServicessrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(OTWEB_ID.AdvancedSearch.ToJson())); // Field OTWEB_ID
            filters.Merge(JObject.Parse(Website.AdvancedSearch.ToJson())); // Field Website
            filters.Merge(JObject.Parse(Link.AdvancedSearch.ToJson())); // Field Link
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "ftblOtherWebServicessrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field OTWEB_ID
            if (filter?.TryGetValue("x_OTWEB_ID", out sv) ?? false) {
                OTWEB_ID.AdvancedSearch.SearchValue = sv;
                OTWEB_ID.AdvancedSearch.SearchOperator = filter["z_OTWEB_ID"];
                OTWEB_ID.AdvancedSearch.SearchCondition = filter["v_OTWEB_ID"];
                OTWEB_ID.AdvancedSearch.SearchValue2 = filter["y_OTWEB_ID"];
                OTWEB_ID.AdvancedSearch.SearchOperator2 = filter["w_OTWEB_ID"];
                OTWEB_ID.AdvancedSearch.Save();
            }

            // Field Website
            if (filter?.TryGetValue("x_Website", out sv) ?? false) {
                Website.AdvancedSearch.SearchValue = sv;
                Website.AdvancedSearch.SearchOperator = filter["z_Website"];
                Website.AdvancedSearch.SearchCondition = filter["v_Website"];
                Website.AdvancedSearch.SearchValue2 = filter["y_Website"];
                Website.AdvancedSearch.SearchOperator2 = filter["w_Website"];
                Website.AdvancedSearch.Save();
            }

            // Field Link
            if (filter?.TryGetValue("x_Link", out sv) ?? false) {
                Link.AdvancedSearch.SearchValue = sv;
                Link.AdvancedSearch.SearchOperator = filter["z_Link"];
                Link.AdvancedSearch.SearchCondition = filter["v_Link"];
                Link.AdvancedSearch.SearchValue2 = filter["y_Link"];
                Link.AdvancedSearch.SearchOperator2 = filter["w_Link"];
                Link.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Website, ctrl); // Website
        ...
        public async Task RenderListOptions()
        {
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblOtherWebServiceslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblOtherWebServiceslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(OTWEB_ID.CurrentValue) + "\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""tblOtherWebServices"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ftblOtherWebServiceslist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ftblOtherWebServicessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ftblOtherWebServicessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ftblOtherWebServiceslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_tblOtherWebServices");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(tblOtherWebServicesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(tblOtherWebServicesList.RowCount) + "_tblOtherWebServices");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(tblOtherWebServicesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && tblOtherWebServicesList.RowType == RowType.Add || IsEdit && tblOtherWebServicesList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!IsNull(Link.CurrentValue)) {
                    Website.HrefValue = ConvertToString(!Empty(Link.ViewValue) && !IsList(Link.ViewValue) ? RemoveHtml(ConvertToString(Link.ViewValue)) : ConvertToString(Link.CurrentValue)); // Add prefix/suffix
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ftblOtherWebServicessrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
