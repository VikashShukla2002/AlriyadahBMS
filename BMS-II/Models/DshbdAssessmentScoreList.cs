...
public partial class JUN26 {
    /// <summary>
    /// dshbdAssessmentScoreList
    /// </summary>
    public static DshbdAssessmentScoreList dshbdAssessmentScoreList
    {
        get => HttpData.Get<DshbdAssessmentScoreList>("dshbdAssessmentScoreList")!;
        set => HttpData["dshbdAssessmentScoreList"] = value;
    }

    /// <summary>
    /// Page class for dshbdAssessmentScore
    /// </summary>
    public class DshbdAssessmentScoreList : DshbdAssessmentScoreListBase
    {
        // Constructor
        public DshbdAssessmentScoreList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public DshbdAssessmentScoreList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class DshbdAssessmentScoreListBase : DshbdAssessmentScore
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "dshbdAssessmentScore";

        // Page object name
        public string PageObjName = "dshbdAssessmentScoreList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fdshbdAssessmentScorelist";
        ...
        // Constructor
        public DshbdAssessmentScoreListBase()
        {
            // CSS class name as context
            TableVar = "dshbdAssessmentScore";
            ...
            // Table object (dshbdAssessmentScore)
            if (dshbdAssessmentScore == null || dshbdAssessmentScore is DshbdAssessmentScore)
                dshbdAssessmentScore = this;

            // Initialize URLs
            AddUrl = "DshbdAssessmentScoreAdd";
            ...
            MultiDeleteUrl = "DshbdAssessmentScoreDelete";
            MultiUpdateUrl = "DshbdAssessmentScoreUpdate";
        ...
        // Page name
        public string PageName => "DshbdAssessmentScoreList";
        ...
        // Constructor
        public DshbdAssessmentScoreListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "DshbdAssessmentScoreView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fdshbdAssessmentScoregrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                dshbdAssessmentScoreList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fdshbdAssessmentScoresrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fdshbdAssessmentScoresrch", filters);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Evaluation_Score, ctrl); // Evaluation_Score
                UpdateSort(Immediate_Failure_Score, ctrl); // Immediate_Failure_Score
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdAssessmentScorelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdAssessmentScorelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fdshbdAssessmentScorelist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fdshbdAssessmentScoresrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fdshbdAssessmentScoresrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fdshbdAssessmentScorelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_dshbdAssessmentScore");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(dshbdAssessmentScoreList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(dshbdAssessmentScoreList.RowCount) + "_dshbdAssessmentScore");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(dshbdAssessmentScoreList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && dshbdAssessmentScoreList.RowType == RowType.Add || IsEdit && dshbdAssessmentScoreList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fdshbdAssessmentScoresrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
