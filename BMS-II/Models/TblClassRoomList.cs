...
public partial class JUN26 {
    ...
    /// tblClassRoomList
    /// </summary>
    public static TblClassRoomList tblClassRoomList
    {
        get => HttpData.Get<TblClassRoomList>("tblClassRoomList")!;
        set => HttpData["tblClassRoomList"] = value;
    }

    /// <summary>
    /// Page class for tblClassRoom
    /// </summary>
    public class TblClassRoomList : TblClassRoomListBase
    {
        // Constructor
        public TblClassRoomList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblClassRoomList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class TblClassRoomListBase : TblClassRoom
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "tblClassRoomList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ftblClassRoomlist";
        ...
        public TblClassRoomListBase()
        {
            ...
            MultiDeleteUrl = "TblClassRoomDelete";
            MultiUpdateUrl = "TblClassRoomUpdate";
        ...
        // Page name
        public string PageName => "TblClassRoomList";
        ...
        public void SetVisibility()
        {
            ...
            int_MU_Size.Visible = false;
            int_CR_Status.SetVisibility();
            Is_Web_SignUp.Visible = false;
            int_TotSession.Visible = false;
            int_PerDaySession.Visible = false;
            int_Location_ID.SetVisibility();
            int_Teacher_ID.SetVisibility();
            str_CR_Notes.Visible = false;
            is_ZoomMeet.Visible = false;
            ...
            BulkCR_Set.Visible = false;
            str_Username.Visible = false;
            Calendar_ID.Visible = false;
            int_Day_Incremental.Visible = false;
            int_Reoccurrence.Visible = false;
            date_Start2.Visible = false;
            date_Start3.Visible = false;
            date_Start4.Visible = false;
            str_CR_Days.Visible = false;
        }

        // Constructor
        public TblClassRoomListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected async Task InlineEditMode() { // DN
            ...
            } else if (Get("int_CR_ID", out qv)) {
                int_CR_ID.QueryValue = qv;
            } else {
                inlineEdit = false;
            }
            if (inlineEdit) {
        ...
        {
            if (!SameString(int_CR_ID.OldValue, int_CR_ID.CurrentValue))
                return false;
            return true;
        }
        ...
        {
            str_CR_Number.ClearErrorMessage();
            int_CR_Product_ID.ClearErrorMessage();
            date_Start.ClearErrorMessage();
            mon_CR_Price.ClearErrorMessage();
            int_CR_Size.ClearErrorMessage();
            int_CR_Status.ClearErrorMessage();
            int_Location_ID.ClearErrorMessage();
            int_Teacher_ID.ClearErrorMessage();
        }
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "ftblClassRoomsrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "ftblClassRoomsrch", filters);
        ...
        protected async Task SetupListOptions() {
            ...
            item.CssClass = "text-nowrap";
            item.Visible = Security.CanAdd && (IsAdd);
            item.OnLeft = MultiColumnLayout == "cards" ? false : true;
        ...
        public async Task RenderListOptions()
        {
            ...
            if (IsInlineAddRow || IsInlineCopyRow) { // Inline Add/Copy
                ...
                if (UseAjaxActions) {
                    ...
                    listOption?.SetBody($@"<div{divClass}>" +
                            $@"<button class=""ew-grid-link ew-inline-insert"" title=""{insertTitle}"" data-caption=""{insertTitle}"" form=""ftblClassRoomlist"" formaction=""{inlineInsertUrl}"">{insertCaption}</button>" +
            ...
            if (IsInlineEditRow) { // Inline-Edit
                ListOptions.CustomItem = "edit"; // Show edit column only
                    ...
                    if (UseAjaxActions) {
                        ...
                        listOption?.SetBody($@"<div{divClass}>" +
                            $@"<button class=""ew-grid-link ew-inline-update"" title=""{updateTitle}"" data-caption=""{updateTitle}"" form=""ftblClassRoomlist"" formaction=""{inlineUpdateUrl}"">{updateCaption}</button>" +
                ...
                listOption?.AddBody("<input type=\"hidden\" name=\"" + OldKeyName + "\" id=\"" + OldKeyName + "\" value=\"" + HtmlEncode(int_CR_ID.CurrentValue) + "\">");
                return;
            }
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""tblClassRoom"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""tblClassRoom"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblClassRoomlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblClassRoomlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""tblClassRoom"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Visible = InlineAddUrl != "" && Security.CanAdd;
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ftblClassRoomlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ftblClassRoomsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ftblClassRoomsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ftblClassRoomlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_tblClassRoom");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(tblClassRoomList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(tblClassRoomList.RowCount) + "_tblClassRoom");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(tblClassRoomList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && tblClassRoomList.RowType == RowType.Add || IsEdit && tblClassRoomList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected async Task LoadFormValues() {
            ...
            if (!int_CR_ID.IsDetailKey && !IsGridAdd && !IsAdd)
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // int_CR_Product_ID
            int_CR_Product_ID.SetDbValue(rsnew, int_CR_Product_ID.CurrentValue, int_CR_Product_ID.ReadOnly);

            // date_Start
            date_Start.SetDbValue(rsnew, ConvertToDateTime(date_Start.CurrentValue, date_Start.FormatPattern), date_Start.ReadOnly);

            // mon_CR_Price
            mon_CR_Price.SetDbValue(rsnew, mon_CR_Price.CurrentValue, mon_CR_Price.ReadOnly);

            // int_CR_Size
            int_CR_Size.SetDbValue(rsnew, int_CR_Size.CurrentValue, int_CR_Size.ReadOnly);

            // int_CR_Status
            int_CR_Status.SetDbValue(rsnew, int_CR_Status.CurrentValue, int_CR_Status.ReadOnly);

            // int_Location_ID
            int_Location_ID.SetDbValue(rsnew, int_Location_ID.CurrentValue, int_Location_ID.ReadOnly);

            // int_Teacher_ID
            int_Teacher_ID.SetDbValue(rsnew, int_Teacher_ID.CurrentValue, int_Teacher_ID.ReadOnly);
        ...
        public string GetRowHash(Dictionary<string, object>? row)
        {
            if (row == null)
                return "";
            string hash = "";
            hash += GetFieldHash(row["int_CR_Product_ID"], DataType.Number); // int_CR_Product_ID
            hash += GetFieldHash(row["date_Start"], DataType.Date); // date_Start
            hash += GetFieldHash(row["mon_CR_Price"], DataType.Number); // mon_CR_Price
            hash += GetFieldHash(row["int_CR_Size"], DataType.Number); // int_CR_Size
            hash += GetFieldHash(row["int_CR_Status"], DataType.Number); // int_CR_Status
            hash += GetFieldHash(row["int_Location_ID"], DataType.Number); // int_Location_ID
            hash += GetFieldHash(row["int_Teacher_ID"], DataType.Number); // int_Teacher_ID
            return hash;
        }
        ...
        public string GetRowHash(DbDataReader? dr)
        {
            if (dr == null)
                return "";
            var row = new Dictionary<string, object>();
            row.Add("int_CR_Product_ID", dr["int_CR_Product_ID"]); // int_CR_Product_ID
            row.Add("date_Start", dr["date_Start"]); // date_Start
            row.Add("mon_CR_Price", dr["mon_CR_Price"]); // mon_CR_Price
            row.Add("int_CR_Size", dr["int_CR_Size"]); // int_CR_Size
            row.Add("int_CR_Status", dr["int_CR_Status"]); // int_CR_Status
            row.Add("int_Location_ID", dr["int_Location_ID"]); // int_Location_ID
            row.Add("int_Teacher_ID", dr["int_Teacher_ID"]); // int_Teacher_ID
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                // str_CR_Number
                str_CR_Number.SetDbValue(rsnew, str_CR_Number.CurrentValue);
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ftblClassRoomsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
