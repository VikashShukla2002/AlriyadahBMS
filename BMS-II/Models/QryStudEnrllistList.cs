...
public partial class JUN26 {
    ...
    /// qryStudEnrllistList
    /// </summary>
    public static QryStudEnrllistList qryStudEnrllistList
    {
        get => HttpData.Get<QryStudEnrllistList>("qryStudEnrllistList")!;
        set => HttpData["qryStudEnrllistList"] = value;
    }

    /// <summary>
    /// Page class for qryStudEnrllist
    /// </summary>
    public class QryStudEnrllistList : QryStudEnrllistListBase
    {
        // Constructor
        public QryStudEnrllistList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public QryStudEnrllistList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class QryStudEnrllistListBase : QryStudEnrllist
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "qryStudEnrllist";

        // Page object name
        public string PageObjName = "qryStudEnrllistList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fqryStudEnrllistlist";
        ...
        // Constructor
        public QryStudEnrllistListBase()
        {
            // CSS class name as context
            TableVar = "qryStudEnrllist";
            ...
            // Table object (qryStudEnrllist)
            if (qryStudEnrllist == null || qryStudEnrllist is QryStudEnrllist)
                qryStudEnrllist = this;

            // Initialize URLs
            AddUrl = "QryStudEnrllistAdd";
            ...
            MultiDeleteUrl = "QryStudEnrllistDelete";
            MultiUpdateUrl = "QryStudEnrllistUpdate";
        ...
        // Page name
        public string PageName => "QryStudEnrllistList";
        ...
        // Constructor
        public QryStudEnrllistListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "QryStudEnrllistView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                qryStudEnrllistList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fqryStudEnrllistsrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fqryStudEnrllistsrch", filters);
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryStudEnrllistlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryStudEnrllistlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fqryStudEnrllistlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fqryStudEnrllistsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fqryStudEnrllistsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fqryStudEnrllistlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_qryStudEnrllist");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(qryStudEnrllistList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(qryStudEnrllistList.RowCount) + "_qryStudEnrllist");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(qryStudEnrllistList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && qryStudEnrllistList.RowType == RowType.Add || IsEdit && qryStudEnrllistList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fqryStudEnrllistsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
