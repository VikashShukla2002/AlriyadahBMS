...
public partial class JUN26 {
    /// <summary>
    /// userLevelPermissionsList
    /// </summary>
    public static UserLevelPermissionsList userLevelPermissionsList
    {
        get => HttpData.Get<UserLevelPermissionsList>("userLevelPermissionsList")!;
        set => HttpData["userLevelPermissionsList"] = value;
    }

    /// <summary>
    /// Page class for UserLevelPermissions
    /// </summary>
    public class UserLevelPermissionsList : UserLevelPermissionsListBase
    {
        // Constructor
        public UserLevelPermissionsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public UserLevelPermissionsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class UserLevelPermissionsListBase : UserLevelPermissions
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "UserLevelPermissions";
        ...
        // Constructor
        public UserLevelPermissionsListBase()
        {
            ...
            if (userLevelPermissions == null || userLevelPermissions is UserLevelPermissions)
                userLevelPermissions = this;

            // Initialize URLs
            AddUrl = "UserLevelPermissionsAdd";
            ...
            MultiDeleteUrl = "UserLevelPermissionsDelete";
            MultiUpdateUrl = "UserLevelPermissionsUpdate";
        ...
        // Page name
        public string PageName => "UserLevelPermissionsList";
        ...
        public void SetVisibility()
        {
            ...
            _TableName.SetVisibility();
            Permission.SetVisibility();
        }

        // Constructor
        public UserLevelPermissionsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "UserLevelPermissionsView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            ...
            key += UrlEncode(ConvertToString(dict.ContainsKey("TableName") ? dict["TableName"] : _TableName.CurrentValue));
            return key;
        }
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                userLevelPermissionsList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fUserLevelPermissionssrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fUserLevelPermissionssrch", filters);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(_TableName, ctrl); // TableName
                UpdateSort(Permission, ctrl); // Permission
        ...
        public async Task RenderListOptions()
        {
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelPermissionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelPermissionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(UserLevelID.CurrentValue + Config.CompositeKeySeparator + _TableName.CurrentValue) + "\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fUserLevelPermissionslist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fUserLevelPermissionssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fUserLevelPermissionssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fUserLevelPermissionslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_UserLevelPermissions");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(userLevelPermissionsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(userLevelPermissionsList.RowCount) + "_UserLevelPermissions");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(userLevelPermissionsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && userLevelPermissionsList.RowType == RowType.Add || IsEdit && userLevelPermissionsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("TableName", _TableName.DefaultValue ?? DbNullValue); // DN
            row.Add("Permission", Permission.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fUserLevelPermissionssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
