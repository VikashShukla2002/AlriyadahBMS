...
public partial class JUN26 {
    ...
    /// </summary>
    [MaybeNull]
    public static DshbdClasses dshbdClasses
    {
        get => HttpData.Resolve<DshbdClasses>("dshbdClasses");
        set => HttpData["dshbdClasses"] = value;
    }

    /// <summary>
    /// Table class for dshbdClasses
    /// </summary>
    public class DshbdClasses : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<SqlDbType> ClassStatus;

        public readonly DbField<SqlDbType> str_Package_Name;

        public readonly DbField<SqlDbType> Numberofopenclasses;
        ...
        public DshbdClasses()
        {
            ...
            // Class Status
            ClassStatus = new (this, "x_ClassStatus", 200, SqlDbType.VarChar) {
                Name = "Class Status",
                Expression = "[Class Status]",
                UseBasicSearch = true,
                BasicSearchExpression = "[Class Status]",
                DateTimeFormat = -1,
                VirtualExpression = "[Class Status]",
                IsVirtual = false,
                ...
                CustomMessage = Language.FieldPhrase("dshbdClasses", "ClassStatus", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Class Status", ClassStatus);
            ...
            str_Package_Name = new (this, "x_str_Package_Name", 202, SqlDbType.NVarChar) {
                ...
                CustomMessage = Language.FieldPhrase("dshbdClasses", "str_Package_Name", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("str_Package_Name", str_Package_Name);

            // Number of open classes
            Numberofopenclasses = new (this, "x_Numberofopenclasses", 3, SqlDbType.Int) {
                Name = "Number of open classes",
                Expression = "[Number of open classes]",
                BasicSearchExpression = "CAST([Number of open classes] AS NVARCHAR)",
                DateTimeFormat = -1,
                VirtualExpression = "[Number of open classes]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("dshbdClasses", "Numberofopenclasses", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("Number of open classes", Numberofopenclasses);
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            if (row == null)
                return;
            try {
                ClassStatus.DbValue = row["Class Status"]; // Set DB value only
                str_Package_Name.DbValue = row["str_Package_Name"]; // Set DB value only
                Numberofopenclasses.DbValue = row["Number of open classes"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "DshbdClassesView"))
                return Language.Phrase("View");
            else if (SameString(pageName, "DshbdClassesEdit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "DshbdClassesAdd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "DshbdClassesView",
                Config.ApiAddAction => "DshbdClassesAdd",
                Config.ApiEditAction => "DshbdClassesEdit",
                Config.ApiDeleteAction => "DshbdClassesDelete",
                Config.ApiListAction => "DshbdClassesList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "DshbdClassesList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("DshbdClassesView", parm);
            else
                url = KeyUrl("DshbdClassesView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "DshbdClassesAdd";
        ...
        {
            string url = "";
            url = KeyUrl("DshbdClassesEdit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("DshbdClassesList", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("DshbdClassesAdd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("DshbdClassesList", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("DshbdClassesDelete")); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            if (dr == null)
                return;
            ClassStatus.SetDbValue(dr["Class Status"]);
            str_Package_Name.SetDbValue(dr["str_Package_Name"]);
            Numberofopenclasses.SetDbValue(dr["Number of open classes"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "DshbdClassesList";
        ...
        public async Task RenderEditRow()
        {
            ...
            // Class Status
            ClassStatus.SetupEditAttributes();
            if (!ClassStatus.Raw)
                ClassStatus.CurrentValue = HtmlDecode(ClassStatus.CurrentValue);
            ClassStatus.EditValue = HtmlEncode(ClassStatus.CurrentValue);
            ClassStatus.PlaceHolder = RemoveHtml(ClassStatus.Caption);
            ...
            // Number of open classes
            Numberofopenclasses.SetupEditAttributes();
            Numberofopenclasses.EditValue = Numberofopenclasses.CurrentValue; // DN
            Numberofopenclasses.PlaceHolder = RemoveHtml(Numberofopenclasses.Caption);
            if (!Empty(Numberofopenclasses.EditValue) && IsNumeric(Numberofopenclasses.EditValue))
                Numberofopenclasses.EditValue = FormatNumber(Numberofopenclasses.EditValue, null);
        ...
        public async Task ExportDocument(dynamic doc, DbDataReader dataReader, int startRec, int stopRec, string exportType = "")
        {
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    doc.BeginExportRow();
                    if (exportType == "view") {
                        doc.ExportCaption(ClassStatus);
                        doc.ExportCaption(str_Package_Name);
                        doc.ExportCaption(Numberofopenclasses);
                    } else {
                        doc.ExportCaption(ClassStatus);
                        doc.ExportCaption(str_Package_Name);
                        doc.ExportCaption(Numberofopenclasses);
            ...
            do { // DN
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            await doc.ExportField(ClassStatus);
                            await doc.ExportField(str_Package_Name);
                            await doc.ExportField(Numberofopenclasses);
                        } else {
                            await doc.ExportField(ClassStatus);
                            await doc.ExportField(str_Package_Name);
                            await doc.ExportField(Numberofopenclasses);
