...
public partial class JUN26 {
    ...
    /// tblCrSessionsList
    /// </summary>
    public static TblCrSessionsList tblCrSessionsList
    {
        get => HttpData.Get<TblCrSessionsList>("tblCrSessionsList")!;
        set => HttpData["tblCrSessionsList"] = value;
    }

    /// <summary>
    /// Page class for tblCRSessions
    /// </summary>
    public class TblCrSessionsList : TblCrSessionsListBase
    {
        // Constructor
        public TblCrSessionsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TblCrSessionsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TblCrSessionsListBase : TblCrSessions
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "tblCrSessionsList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ftblCRSessionslist";
        ...
        public TblCrSessionsListBase()
        {
            ...
            MultiDeleteUrl = "TblCrSessionsDelete";
            MultiUpdateUrl = "TblCrSessionsUpdate";
        ...
        // Page name
        public string PageName => "TblCrSessionsList";
        ...
        // Constructor
        public TblCrSessionsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "ftblCRSessionssrch");
            ...
            filters.Merge(JObject.Parse(Instructor.AdvancedSearch.ToJson())); // Field Instructor
            filters.Merge(JObject.Parse(int_Location_Id.AdvancedSearch.ToJson())); // Field int_Location_Id
            filters.Merge(JObject.Parse(Location.AdvancedSearch.ToJson())); // Field Location
            filters.Merge(JObject.Parse(int_Session_ID.AdvancedSearch.ToJson())); // Field int_Session_ID
            filters.Merge(JObject.Parse(str_Session_Date.AdvancedSearch.ToJson())); // Field str_Session_Date
            filters.Merge(JObject.Parse(int_Day_ID.AdvancedSearch.ToJson())); // Field int_Day_ID
            filters.Merge(JObject.Parse(str_Day_Name.AdvancedSearch.ToJson())); // Field str_Day_Name
            filters.Merge(JObject.Parse(str_Time_Start.AdvancedSearch.ToJson())); // Field str_Time_Start
            filters.Merge(JObject.Parse(str_Time_End.AdvancedSearch.ToJson())); // Field str_Time_End
            ...
            filters.Merge(JObject.Parse(str_Session_Notes.AdvancedSearch.ToJson())); // Field str_Session_Notes
            filters.Merge(JObject.Parse(is_Rescheduled.AdvancedSearch.ToJson())); // Field is_Rescheduled
            filters.Merge(JObject.Parse(IsZoomMeet.AdvancedSearch.ToJson())); // Field IsZoomMeet
            filters.Merge(JObject.Parse(str_ZoomHostUrl.AdvancedSearch.ToJson())); // Field str_ZoomHostUrl
            filters.Merge(JObject.Parse(str_ZoomUserUrl.AdvancedSearch.ToJson())); // Field str_ZoomUserUrl
            filters.Merge(JObject.Parse(CR_Row_Index.AdvancedSearch.ToJson())); // Field CR_Row_Index
            ...
            filters.Merge(JObject.Parse(str_Status.AdvancedSearch.ToJson())); // Field str_Status
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "ftblCRSessionssrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Instructor
            if (filter?.TryGetValue("x_Instructor", out sv) ?? false) {
                Instructor.AdvancedSearch.SearchValue = sv;
                Instructor.AdvancedSearch.SearchOperator = filter["z_Instructor"];
                Instructor.AdvancedSearch.SearchCondition = filter["v_Instructor"];
                Instructor.AdvancedSearch.SearchValue2 = filter["y_Instructor"];
                Instructor.AdvancedSearch.SearchOperator2 = filter["w_Instructor"];
                Instructor.AdvancedSearch.Save();
            }
            ...
            // Field Location
            if (filter?.TryGetValue("x_Location", out sv) ?? false) {
                Location.AdvancedSearch.SearchValue = sv;
                Location.AdvancedSearch.SearchOperator = filter["z_Location"];
                Location.AdvancedSearch.SearchCondition = filter["v_Location"];
                Location.AdvancedSearch.SearchValue2 = filter["y_Location"];
                Location.AdvancedSearch.SearchOperator2 = filter["w_Location"];
                Location.AdvancedSearch.Save();
            }

            // Field int_Session_ID
            if (filter?.TryGetValue("x_int_Session_ID", out sv) ?? false) {
                int_Session_ID.AdvancedSearch.SearchValue = sv;
                int_Session_ID.AdvancedSearch.SearchOperator = filter["z_int_Session_ID"];
                int_Session_ID.AdvancedSearch.SearchCondition = filter["v_int_Session_ID"];
                int_Session_ID.AdvancedSearch.SearchValue2 = filter["y_int_Session_ID"];
                int_Session_ID.AdvancedSearch.SearchOperator2 = filter["w_int_Session_ID"];
                int_Session_ID.AdvancedSearch.Save();
            }

            // Field str_Session_Date
            if (filter?.TryGetValue("x_str_Session_Date", out sv) ?? false) {
                str_Session_Date.AdvancedSearch.SearchValue = sv;
                str_Session_Date.AdvancedSearch.SearchOperator = filter["z_str_Session_Date"];
                str_Session_Date.AdvancedSearch.SearchCondition = filter["v_str_Session_Date"];
                str_Session_Date.AdvancedSearch.SearchValue2 = filter["y_str_Session_Date"];
                str_Session_Date.AdvancedSearch.SearchOperator2 = filter["w_str_Session_Date"];
                str_Session_Date.AdvancedSearch.Save();
            }

            // Field int_Day_ID
            if (filter?.TryGetValue("x_int_Day_ID", out sv) ?? false) {
                int_Day_ID.AdvancedSearch.SearchValue = sv;
                int_Day_ID.AdvancedSearch.SearchOperator = filter["z_int_Day_ID"];
                int_Day_ID.AdvancedSearch.SearchCondition = filter["v_int_Day_ID"];
                int_Day_ID.AdvancedSearch.SearchValue2 = filter["y_int_Day_ID"];
                int_Day_ID.AdvancedSearch.SearchOperator2 = filter["w_int_Day_ID"];
                int_Day_ID.AdvancedSearch.Save();
            }

            // Field str_Day_Name
            if (filter?.TryGetValue("x_str_Day_Name", out sv) ?? false) {
                str_Day_Name.AdvancedSearch.SearchValue = sv;
                str_Day_Name.AdvancedSearch.SearchOperator = filter["z_str_Day_Name"];
                str_Day_Name.AdvancedSearch.SearchCondition = filter["v_str_Day_Name"];
                str_Day_Name.AdvancedSearch.SearchValue2 = filter["y_str_Day_Name"];
                str_Day_Name.AdvancedSearch.SearchOperator2 = filter["w_str_Day_Name"];
                str_Day_Name.AdvancedSearch.Save();
            }

            // Field str_Time_Start
            if (filter?.TryGetValue("x_str_Time_Start", out sv) ?? false) {
                str_Time_Start.AdvancedSearch.SearchValue = sv;
                str_Time_Start.AdvancedSearch.SearchOperator = filter["z_str_Time_Start"];
                str_Time_Start.AdvancedSearch.SearchCondition = filter["v_str_Time_Start"];
                str_Time_Start.AdvancedSearch.SearchValue2 = filter["y_str_Time_Start"];
                str_Time_Start.AdvancedSearch.SearchOperator2 = filter["w_str_Time_Start"];
                str_Time_Start.AdvancedSearch.Save();
            }

            // Field str_Time_End
            if (filter?.TryGetValue("x_str_Time_End", out sv) ?? false) {
                str_Time_End.AdvancedSearch.SearchValue = sv;
                str_Time_End.AdvancedSearch.SearchOperator = filter["z_str_Time_End"];
                str_Time_End.AdvancedSearch.SearchCondition = filter["v_str_Time_End"];
                str_Time_End.AdvancedSearch.SearchValue2 = filter["y_str_Time_End"];
                str_Time_End.AdvancedSearch.SearchOperator2 = filter["w_str_Time_End"];
                str_Time_End.AdvancedSearch.Save();
            }
            ...
            // Field str_Session_Notes
            if (filter?.TryGetValue("x_str_Session_Notes", out sv) ?? false) {
                str_Session_Notes.AdvancedSearch.SearchValue = sv;
                str_Session_Notes.AdvancedSearch.SearchOperator = filter["z_str_Session_Notes"];
                str_Session_Notes.AdvancedSearch.SearchCondition = filter["v_str_Session_Notes"];
                str_Session_Notes.AdvancedSearch.SearchValue2 = filter["y_str_Session_Notes"];
                str_Session_Notes.AdvancedSearch.SearchOperator2 = filter["w_str_Session_Notes"];
                str_Session_Notes.AdvancedSearch.Save();
            }

            // Field is_Rescheduled
            if (filter?.TryGetValue("x_is_Rescheduled", out sv) ?? false) {
                is_Rescheduled.AdvancedSearch.SearchValue = sv;
                is_Rescheduled.AdvancedSearch.SearchOperator = filter["z_is_Rescheduled"];
                is_Rescheduled.AdvancedSearch.SearchCondition = filter["v_is_Rescheduled"];
                is_Rescheduled.AdvancedSearch.SearchValue2 = filter["y_is_Rescheduled"];
                is_Rescheduled.AdvancedSearch.SearchOperator2 = filter["w_is_Rescheduled"];
                is_Rescheduled.AdvancedSearch.Save();
            }

            // Field IsZoomMeet
            if (filter?.TryGetValue("x_IsZoomMeet", out sv) ?? false) {
                IsZoomMeet.AdvancedSearch.SearchValue = sv;
                IsZoomMeet.AdvancedSearch.SearchOperator = filter["z_IsZoomMeet"];
                IsZoomMeet.AdvancedSearch.SearchCondition = filter["v_IsZoomMeet"];
                IsZoomMeet.AdvancedSearch.SearchValue2 = filter["y_IsZoomMeet"];
                IsZoomMeet.AdvancedSearch.SearchOperator2 = filter["w_IsZoomMeet"];
                IsZoomMeet.AdvancedSearch.Save();
            }

            // Field str_ZoomHostUrl
            if (filter?.TryGetValue("x_str_ZoomHostUrl", out sv) ?? false) {
                str_ZoomHostUrl.AdvancedSearch.SearchValue = sv;
                str_ZoomHostUrl.AdvancedSearch.SearchOperator = filter["z_str_ZoomHostUrl"];
                str_ZoomHostUrl.AdvancedSearch.SearchCondition = filter["v_str_ZoomHostUrl"];
                str_ZoomHostUrl.AdvancedSearch.SearchValue2 = filter["y_str_ZoomHostUrl"];
                str_ZoomHostUrl.AdvancedSearch.SearchOperator2 = filter["w_str_ZoomHostUrl"];
                str_ZoomHostUrl.AdvancedSearch.Save();
            }

            // Field str_ZoomUserUrl
            if (filter?.TryGetValue("x_str_ZoomUserUrl", out sv) ?? false) {
                str_ZoomUserUrl.AdvancedSearch.SearchValue = sv;
                str_ZoomUserUrl.AdvancedSearch.SearchOperator = filter["z_str_ZoomUserUrl"];
                str_ZoomUserUrl.AdvancedSearch.SearchCondition = filter["v_str_ZoomUserUrl"];
                str_ZoomUserUrl.AdvancedSearch.SearchValue2 = filter["y_str_ZoomUserUrl"];
                str_ZoomUserUrl.AdvancedSearch.SearchOperator2 = filter["w_str_ZoomUserUrl"];
                str_ZoomUserUrl.AdvancedSearch.Save();
            }

            // Field CR_Row_Index
            if (filter?.TryGetValue("x_CR_Row_Index", out sv) ?? false) {
                CR_Row_Index.AdvancedSearch.SearchValue = sv;
                CR_Row_Index.AdvancedSearch.SearchOperator = filter["z_CR_Row_Index"];
                CR_Row_Index.AdvancedSearch.SearchCondition = filter["v_CR_Row_Index"];
                CR_Row_Index.AdvancedSearch.SearchValue2 = filter["y_CR_Row_Index"];
                CR_Row_Index.AdvancedSearch.SearchOperator2 = filter["w_CR_Row_Index"];
                CR_Row_Index.AdvancedSearch.Save();
            }
            ...
            // Field str_Status
            if (filter?.TryGetValue("x_str_Status", out sv) ?? false) {
                str_Status.AdvancedSearch.SearchValue = sv;
                str_Status.AdvancedSearch.SearchOperator = filter["z_str_Status"];
                str_Status.AdvancedSearch.SearchCondition = filter["v_str_Status"];
                str_Status.AdvancedSearch.SearchValue2 = filter["y_str_Status"];
                str_Status.AdvancedSearch.SearchOperator2 = filter["w_str_Status"];
                str_Status.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Instructor);
            searchFlds.Add(Location);
            searchFlds.Add(str_Session_Date);
            searchFlds.Add(str_Day_Name);
            searchFlds.Add(str_Time_Start);
            searchFlds.Add(str_Time_End);
            searchFlds.Add(str_Notes);
            searchFlds.Add(str_Session_Notes);
            searchFlds.Add(str_ZoomHostUrl);
            searchFlds.Add(str_ZoomUserUrl);
            searchFlds.Add(str_Username);
            searchFlds.Add(str_Status);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Instructor, ctrl); // Instructor
                UpdateSort(Location, ctrl); // Location
                UpdateSort(int_Session_ID, ctrl); // int_Session_ID
                UpdateSort(str_Session_Date, ctrl); // str_Session_Date
                UpdateSort(int_Day_ID, ctrl); // int_Day_ID
                UpdateSort(str_Time_Start, ctrl); // str_Time_Start
                UpdateSort(str_Time_End, ctrl); // str_Time_End
                UpdateSort(int_Student_Id, ctrl); // int_Student_Id
                UpdateSort(str_Username, ctrl); // str_Username
                UpdateSort(str_Status, ctrl); // str_Status
        ...
        protected void ResetCommand() {
            ...
            if (Command.ToLower().StartsWith("reset")) {
                ...
                if (SameText(Command, "resetsort")) {
                    ...
                    date_Modified.Sort = "";
                    bit_IsDeleted.Sort = "";
                    str_Session_Notes.Sort = "";
                    is_Rescheduled.Sort = "";
                    IsZoomMeet.Sort = "";
                    str_ZoomHostUrl.Sort = "";
                    str_ZoomUserUrl.Sort = "";
                    CR_Row_Index.Sort = "";
                    CRSS_ID.Sort = "";
                    int_Student_Id.Sort = "";
                    str_Username.Sort = "";
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""tblCRSessions"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""tblCRSessions"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""tblCRSessions"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblCRSessionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ftblCRSessionslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(int_CRSession_ID.CurrentValue) + "\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""tblCRSessions"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"ftblCRSessionslist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ftblCRSessionssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ftblCRSessionssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ftblCRSessionslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_tblCRSessions");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(tblCrSessionsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(tblCrSessionsList.RowCount) + "_tblCRSessions");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(tblCrSessionsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && tblCrSessionsList.RowType == RowType.Add || IsEdit && tblCrSessionsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ftblCRSessionssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
