...
public partial class JUN26 {
    ...
    /// userLevelsEdit
    /// </summary>
    public static UserLevelsEdit userLevelsEdit
    {
        get => HttpData.Get<UserLevelsEdit>("userLevelsEdit")!;
        set => HttpData["userLevelsEdit"] = value;
    }

    /// <summary>
    /// Page class for UserLevels
    /// </summary>
    public class UserLevelsEdit : UserLevelsEditBase
    {
        // Constructor
        public UserLevelsEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public UserLevelsEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class UserLevelsEditBase : UserLevels
    {
        // Page ID
        ...
        // Page name
        public string PageName => "UserLevelsEdit";
        ...
        // Constructor
        public UserLevelsEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("UserLevelID", out rv)) { // DN
                    UserLevelID.FormValue = UrlDecode(rv); // DN
                    UserLevelID.OldValue = UserLevelID.FormValue;
                } else if (CurrentForm.HasValue("x_UserLevelID")) {
                    UserLevelID.FormValue = CurrentForm.GetValue("x_UserLevelID");
                    UserLevelID.OldValue = UserLevelID.FormValue;
                } else if (!Empty(keyValues)) {
                    UserLevelID.OldValue = ConvertToString(keyValues[0]);
                } else {
        ...
        if (postBack) {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                UserLevelID.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("UserLevelsList"); // No matching record, return to list
                        }
                    break;
        ...
        protected async Task LoadFormValues() {
            ...
            if (CurrentForm.HasValue("o_UserLevelID"))
                UserLevelID.OldValue = CurrentForm.GetValue("o_UserLevelID");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (ConvertToInt(UserLevelID.CurrentValue) == -2 || ConvertToInt(UserLevelID.CurrentValue) == -1 || ConvertToInt(UserLevelID.CurrentValue) == 0)
                    UserLevelName.ReadOnly = true;
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // UserLevelName
            UserLevelName.SetDbValue(rsnew, UserLevelName.CurrentValue, UserLevelName.ReadOnly);

            // Update current values
            SetCurrentValues(rsnew);

            // Check field with unique index (UserLevelID)
            if (!Empty(UserLevelID.CurrentValue)) {
                string filterChk = "([UserLevelID] = " + AdjustSql(UserLevelID.CurrentValue, DbId) + ")";
                filterChk = filterChk + " AND NOT (" + filter + ")";
                try {
                    using var rschk = await LoadReader(filterChk);
                    if (rschk?.HasRows ?? false) {
                        var idxErrMsg = Language.Phrase("DupIndex").Replace("%f", UserLevelID.Caption);
                        idxErrMsg = idxErrMsg.Replace("%v", ConvertToString(UserLevelID.CurrentValue));
