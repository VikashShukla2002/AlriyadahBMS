...
public partial class JUN26 {
    /// <summary>
    /// qryClassroomCldrListList
    /// </summary>
    public static QryClassroomCldrListList qryClassroomCldrListList
    {
        get => HttpData.Get<QryClassroomCldrListList>("qryClassroomCldrListList")!;
        set => HttpData["qryClassroomCldrListList"] = value;
    }

    /// <summary>
    /// Page class for qryClassroomCldrList
    /// </summary>
    public class QryClassroomCldrListList : QryClassroomCldrListListBase
    {
        // Constructor
        public QryClassroomCldrListList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public QryClassroomCldrListList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class QryClassroomCldrListListBase : QryClassroomCldrList
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "qryClassroomCldrList";

        // Page object name
        public string PageObjName = "qryClassroomCldrListList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fqryClassroomCldrListlist";
        ...
        // Constructor
        public QryClassroomCldrListListBase()
        {
            // CSS class name as context
            TableVar = "qryClassroomCldrList";
            ...
            // Table object (qryClassroomCldrList)
            if (qryClassroomCldrList == null || qryClassroomCldrList is QryClassroomCldrList)
                qryClassroomCldrList = this;

            // Initialize URLs
            AddUrl = "QryClassroomCldrListAdd";
            ...
            MultiDeleteUrl = "QryClassroomCldrListDelete";
            MultiUpdateUrl = "QryClassroomCldrListUpdate";
        ...
        // Page name
        public string PageName => "QryClassroomCldrListList";
        ...
        // Constructor
        public QryClassroomCldrListListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "QryClassroomCldrListView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fqryClassroomCldrListgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                qryClassroomCldrListList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryClassroomCldrListlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryClassroomCldrListlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fqryClassroomCldrListlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fqryClassroomCldrListsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = false;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fqryClassroomCldrListsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = false;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fqryClassroomCldrListlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_qryClassroomCldrList");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(qryClassroomCldrListList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(qryClassroomCldrListList.RowCount) + "_qryClassroomCldrList");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(qryClassroomCldrListList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && qryClassroomCldrListList.RowType == RowType.Add || IsEdit && qryClassroomCldrListList.RowType == RowType.Edit) // Inline-Add/Edit row
