...
public partial class JUN26 {
    ...
    /// equipmentEdit
    /// </summary>
    public static EquipmentEdit equipmentEdit
    {
        get => HttpData.Get<EquipmentEdit>("equipmentEdit")!;
        set => HttpData["equipmentEdit"] = value;
    }

    /// <summary>
    /// Page class for Equipment
    /// </summary>
    public class EquipmentEdit : EquipmentEditBase
    {
        // Constructor
        public EquipmentEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public EquipmentEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class EquipmentEditBase : Equipment
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "equipmentEdit";
        ...
        // Page name
        public string PageName => "EquipmentEdit";
        ...
        // Constructor
        public EquipmentEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsApi()) {
                ...
                if (RouteValues.TryGetValue("Equipment_ID", out rv)) { // DN
                    Equipment_ID.FormValue = UrlDecode(rv); // DN
                    Equipment_ID.OldValue = Equipment_ID.FormValue;
                } else if (CurrentForm.HasValue("x_Equipment_ID")) {
                    Equipment_ID.FormValue = CurrentForm.GetValue("x_Equipment_ID");
                    Equipment_ID.OldValue = Equipment_ID.FormValue;
                } else if (!Empty(keyValues)) {
                    Equipment_ID.OldValue = ConvertToString(keyValues[0]);
                } else {
                ...
                if (!Empty(Post("action"))) {
                    ...
                    if (RouteValues.TryGetValue("Equipment_ID", out rv)) { // DN
                        ...
                        Equipment_ID.CurrentValue = DbNullValue;
                    }
                }
        ...
        if (postBack) {
            ...
            if (IsApi() && RouteValues.TryGetValue("key", out object? k)) {
                ...
                Equipment_ID.FormValue = ConvertToString(keyValues[0]);
            }
        }
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("EquipmentList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = "EquipmentList";
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // int_Location
            int_Location.SetDbValue(rsnew, int_Location.CurrentValue, int_Location.ReadOnly);

            // Equipment_Type_ID
            Equipment_Type_ID.SetDbValue(rsnew, Equipment_Type_ID.CurrentValue, Equipment_Type_ID.ReadOnly);

            // Manufacturer_Name
            Manufacturer_Name.SetDbValue(rsnew, Manufacturer_Name.CurrentValue, Manufacturer_Name.ReadOnly);

            // Serial_VIN_Number
            Serial_VIN_Number.SetDbValue(rsnew, Serial_VIN_Number.CurrentValue, Serial_VIN_Number.ReadOnly);

            // Retailer_Name
            Retailer_Name.SetDbValue(rsnew, Retailer_Name.CurrentValue, Retailer_Name.ReadOnly);

            // DatePurchased
            DatePurchased.SetDbValue(rsnew, ConvertToDateTime(DatePurchased.CurrentValue, DatePurchased.FormatPattern), DatePurchased.ReadOnly);

            // ServiceInception_Date
            ServiceInception_Date.SetDbValue(rsnew, ConvertToDateTime(ServiceInception_Date.CurrentValue, ServiceInception_Date.FormatPattern), ServiceInception_Date.ReadOnly);

            // UsefulLife
            UsefulLife.SetDbValue(rsnew, UsefulLife.CurrentValue, UsefulLife.ReadOnly);

            // Price
            Price.SetDbValue(rsnew, Price.CurrentValue, Price.ReadOnly);

            // VAT
            VAT.SetDbValue(rsnew, VAT.CurrentValue, VAT.ReadOnly);
