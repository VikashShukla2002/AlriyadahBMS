...
public partial class JUN26 {
    ...
    /// Page base class
    /// </summary>
    public class ChangePasswordBase : TblStudents
    {
        // Page ID
        ...
        public override async Task<IActionResult> Run()
        {
                ...
                if (dr != null && await dr.ReadAsync()) {
                    if (IsPasswordReset() || ComparePassword(ConvertToString(GetUserInfo(Config.LoginPasswordFieldName, dr)), ConvertToString(OldPassword.CurrentValue))) {
                        ...
                        if (validPwd) {
                            ...
                            emailAddress = ConvertToString(GetUserInfo(Config.UserEmailFieldName, dr));
            ...
            if (pwdUpdated) {
                if (emailAddress != "") {
                    // Load email Content
                    var email = new Email();
                    await email.Load(Config.EmailChangePasswordTemplate);
                    email.ReplaceSender(Config.SenderEmail); // Replace Sender
                    email.ReplaceRecipient(emailAddress); // Replace Recipient
                    email.ReplaceContent("<!--$Password-->", ConvertToString(NewPassword.CurrentValue));
                    bool emailSent = false;
                    if (EmailSending(email, ConvertToDictionary<dynamic>(new { rs = rsnew })))
