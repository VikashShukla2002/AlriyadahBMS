...
public partial class JUN26 {
    ...
    /// <returns></returns>
    public static string ConvertTurkishChars(string str) {
        string ret = str;
        int[] turkishChars = new int[] {0x131, 0x130, 0xFC, 0xDC, 0xF6, 0xD6, 0x15F, 0x15E, 0xE7, 0xC7, 0x11F, 0x11E};
        char[] englishChars = new char[] {'i', 'I', 'u', 'U', 'o', 'O', 's', 'S', 'c', 'C', 'g', 'G'};
        for (int i = 0; i < turkishChars.Length; i++) {
            ret = ret.Replace(Convert.ToChar(turkishChars[i]), englishChars[i]);
        }
        return ret;
    }

    /// <summary>
    /// Convert to Title Case by InvariantCulture (CAPITAL_CASE => CapitalCase, lowercase => Lowercase)
    /// </summary>
    /// <returns>Title Case</returns>
    public static string TitleCaseInvariant(string name) => ConvertTurkishChars(String.Join("", name.Split('_').Select(x => CultureInfo.InvariantCulture.TextInfo.ToTitleCase(x.ToLowerInvariant()))));

    /// <summary>
    /// Convert to Title Case (CAPITAL_CASE => CapitalCase, lowercase => Lowercase)
    /// </summary>
    /// <returns>Title Case</returns>
    public static string TitleCase(string name) => String.Join("", name.Split('_').Select(x => CultureInfo.CurrentCulture.TextInfo.ToTitleCase(x.ToLower())));
    ...
    public static object? GetUserInfo(string fieldName, IDictionary<string, object>? row)
    {
        ...
        if (row != null && row.TryGetValue(fieldName, out object? value)) {
            ...
            if (UserTable.Fields[fieldName]?.IsEncrypt ?? false)
    ...
    public static void SetupLoginStatus()
    {
        ...
        string resetPasswordUrl = GetUrl(resetPasswordPage);
        if (currentPage != resetPasswordPage) {
            if (Config.UseModalResetPassword && !IsMobile()) {
                LoginStatus["resetPassword"] = new Dictionary<string, object>() {
                    { "ew-action", "modal" },
                    { "footer", false },
        ...
        LoginStatus["canResetPassword"] = !Empty(resetPasswordUrl) && !IsLoggedIn();
        string registerPage = "register";
        string registerUrl = GetUrl(registerPage);
        if (currentPage != registerPage) {
            if (Config.UseModalRegister && !IsMobile()) {
                LoginStatus["register"] = new Dictionary<string, object>() {
                    { "ew-action", "modal" },
                    { "btn", "Register" },
        ...
        LoginStatus["canRegister"] = !Empty(registerUrl) && !IsLoggedIn();
        string changePasswordPage = "changepassword";
        string changePasswordUrl = GetUrl(changePasswordPage);
        if (currentPage != changePasswordPage) {
            if (Config.UseModalChangePassword && !IsMobile()) {
                LoginStatus["changePassword"] = new Dictionary<string, object>() {
                    { "ew-action", "modal" },
                    { "footer", false },
        ...
        LoginStatus["canChangePassword"] = !Empty(changePasswordUrl) && IsLoggedIn() && !IsSysAdmin();
        ...
        // Notifications
        LoginStatus["canSubscribe"] = Config.PushAnonymous && !IsLoggedIn() || IsLoggedIn() && !IsSysAdmin();
    ...
    protected static dynamic ResolveConnection(string dbid = "DB", string name = "")
    {
        ...
        return GetConnectionType(dbid).ToUpper() switch {
            "MSSQL" => Container.ResolveNamed<DatabaseConnection<SqlConnection, SqlCommand, SqlDataReader, SqlDbType>>(dbid + name, new NamedParameter("dbid", dbid)),
    ...
    public static string UniqueFileName(string folder, string orifn, bool indexed = false)
    {
        ...
        string name = oldfn;
        if (indexed) {
            Match m = Regex.Match(oldfn, @"\((\d+)\)$");
            if (m.Success) {
                i = ConvertToInt(m.Groups[1].Value); // Start from (n)
                suffix = "(" + i.ToString() + ")";
                name = Regex.Replace(oldfn, @"\(\d+\)$", ""); // Remove "(n)" at the end of the file name
            }
        }
    ...
    /// <returns></returns>
    public static void FixUploadTempFileNames(DbField field)
    {
        if (field.UploadMultiple) {
            string[] newFiles = ConvertToString(field.Upload.FileName).Split(Config.MultipleUploadSeparator);
            string tempPath = UploadTempPath(field, field.Upload.Index);
            for (var i = newFiles.Length - 1; i >= 0; i--) {
                string newFile = newFiles[i];
                if (!Empty(newFile)) {
                    string tempFile = newFile;
                    for (var j = i - 1; j >= 0; j--) { // Temp files with same names
            ...
            field.Upload.FileName = String.Join(ConvertToString(Config.MultipleUploadSeparator), newFiles);
        }
    }
    ...
    /// <param name="field">Field object</param>
    /// <returns></returns>
    public static void FixUploadFileNames(DbField field)
    {
        string[] newFiles = field.UploadMultiple
            ? field.Upload.FileName.Split(Config.MultipleUploadSeparator)
            : new string[] { field.Upload.FileName };
        List<string> oldFiles = Empty(field.Upload.DbValue) ? new () : (field.UploadMultiple
            ? field.HtmlDecode(field.Upload.DbValue).Split(Config.MultipleUploadSeparator).ToList()
            : new () { field.HtmlDecode(field.Upload.DbValue) });
        string tempPath = UploadTempPath(field, field.Upload.Index);
        string workPath = IncludeTrailingDelimiter(tempPath + "__work", true);
        if (!CreateFolder(workPath))
            End("Cannot create folder: " + workPath);
        for (var i = 0; i < newFiles.Length; i++) {
            if (!Empty(newFiles[i])) {
                string file = newFiles[i];
        ...
        //CleanPath(workPath, true);
        field.Upload.DbValue = String.Join(ConvertToString(Config.MultipleUploadSeparator), oldFiles);
        field.Upload.FileName = String.Join(ConvertToString(Config.MultipleUploadSeparator), newFiles);
    }
    ...
    /// </summary>
    /// <param name="field">Field object</param>
    /// <param name="fileNames">File names (possibly changed by server event)</param>
    /// <returns></returns>
    public static async Task<bool> SaveUploadFiles(DbField field, string fileNames, bool resize)
    {
        string tempPath = UploadTempPath(field, field.Upload.Index);
        string[] newFiles = field.UploadMultiple
            ? field.Upload.FileName.Split(Config.MultipleUploadSeparator)
            : new string[] { field.Upload.FileName };
        if (!Empty(field.Upload.FileName)) {
            if (SameString(field.Upload.FileName, fileNames)) // Not changed in server event
                fileNames = "";
            string[] newFiles2 = Empty(fileNames) ? new string[] {} : (field.UploadMultiple
                ? fileNames.Split(Config.MultipleUploadSeparator)
                : new string[] { fileNames });
            for (var i = 0; i < newFiles.Length; i++) {
                string newFile = newFiles[i];
                string newFile2 = newFiles2.Length > i ? newFiles2[i] : "";
                if (!Empty(newFile)) {
                    string file = tempPath + newFile;
                    if (FileExists(file)) {
                        if (!Empty(newFile2)) // Use correct file name
                            newFile = newFile2;
                        bool result = resize
                            ? await field.Upload.ResizeAndSaveToFile(field.ImageWidth, field.ImageHeight, newFile, true, i) // Resize
                            : await field.Upload.SaveToFile(newFile, true, i); // Save
                        if (!result)
                            return false;
                    }
                }
        ...
        if (Config.DeleteUploadFiles) {
            string[] oldFiles = Empty(field.Upload.DbValue) ? new string[] {} : (field.UploadMultiple
                ? field.HtmlDecode(field.Upload.DbValue).Split(Config.MultipleUploadSeparator)
                : new string[] { field.HtmlDecode(field.Upload.DbValue) });
            for (int i = 0; i < oldFiles.Length; i++) {
                string oldFile = oldFiles[i];
                if (!Empty(oldFile) && !newFiles.ToList().Contains(oldFile))
                    DeleteFile(field.OldPhysicalUploadPath + oldFile);
            }
        }
        return true;
    }
    ...
    public static string CurrentPageName() => (RouteValues != null)
        ? ConvertToString(IsApi() ? Config.ApiUrl + RouteValues["controller"] : RouteValues["action"]) // Returns action (file name) only
        : "";
    ...
    public static object? Invoke(string name, object?[]? parameters = null) =>
        typeof(JUN26).GetMethod(name)?.Invoke(null, parameters);
    ...
    public static object? GetPropertyValue(string name) =>
        typeof(JUN26).GetProperty(name, BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy | BindingFlags.Instance)?.GetValue(null);
    ...
    public static bool ValidApiRequest()
    {
        if (IsApi()) {
            ...
            if (!IsLoggedIn() && !Empty(ClaimValue(ClaimTypes.Name)))
                Security.LoginUser(ClaimValue(ClaimTypes.Name), ClaimValue("Id"), ClaimValue("ParentUserId"), ClaimValue("UserLevelId") ?? "-2", ConvertToInt(ClaimValue("Permission")));
    ...
    public static List<Dictionary<string, object>> GetDropDownEditValue(ReportField fld, object v) { // DN
        ...
        if (val != "") {
            string[] arwrk = fld.IsMultiSelect ? val.Split(Config.MultipleOptionSeparator) : new string[] { val };
    ...
    // User Table
    public static TblStudents UserTable
    {
        get => HttpData.Get<TblStudents>("USER_TABLE")!;
        set => HttpData["USER_TABLE"] = value;
    }

    // User Table Conn
    public static DatabaseConnectionBase<SqlConnection, SqlCommand, SqlDataReader, SqlDbType> UserTableConn
    {
        get => HttpData.Get<DatabaseConnectionBase<SqlConnection, SqlCommand, SqlDataReader, SqlDbType>>("USER_TABLE_CONN")!;
        set => HttpData["USER_TABLE_CONN"] = value;
    }
}
