...
public partial class JUN26 {
    ...
    /// </summary>
    public class UserLevelPermissions : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<SqlDbType> _TableName;

        public readonly DbField<SqlDbType> Permission;
        ...
        public UserLevelPermissions()
        {
            ...
            Type = "TABLE";
            UpdateTable = "dbo.UserLevelPermissions"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            // TableName
            _TableName = new (this, "x__TableName", 202, SqlDbType.NVarChar) {
                Name = "TableName",
            ...
            // Permission
            Permission = new (this, "x_Permission", 3, SqlDbType.Int) {
                Name = "Permission",
                Expression = "[Permission]",
                BasicSearchExpression = "CAST([Permission] AS NVARCHAR)",
                DateTimeFormat = -1,
        ...
        {
            get => _sqlFrom ?? "dbo.UserLevelPermissions";
            set => _sqlFrom = value;
        }
        ...
        public async Task<int> UpdateAsync(object data, object? where, Dictionary<string, object>? rsold)
        {
            ...
            if (result > 0 && AuditTrailOnEdit && rsold != null) {
                ...
                if (!rsaudit.ContainsKey("TableName"))
                    rsaudit.Add("TableName", rsold["TableName"]);
        ...
        public string GetRecordFilter(Dictionary<string, object>? row = null, bool current = false)
        {
            ...
            val = row?.TryGetValue("TableName", out result) ?? false
                ? result
                : !Empty(_TableName.OldValue) && !current ? _TableName.OldValue : _TableName.CurrentValue;
            if (val == null)
                return "0=1"; // Invalid key
            keyFilter = keyFilter.Replace("@_TableName@", AdjustSql(val, DbId)); // Replace key value
            return keyFilter;
        }
        ...
        public Dictionary<string, object>? GetRowFilterAsDictionary(IDictionary<string, object>? row = null)
        {
            ...
            val = row?.TryGetValue("TableName", out result) ?? false
                ? result
                : !Empty(_TableName.OldValue) ? _TableName.OldValue : _TableName.CurrentValue;
            if (Empty(val))
                return null; // Invalid key
            keyFilter.Add("TableName", val); // Add key value
        ...
        {
            if (SameString(pageName, "UserLevelPermissionsView"))
                return Language.Phrase("View");
            else if (SameString(pageName, "UserLevelPermissionsEdit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "UserLevelPermissionsAdd"))
        ...
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ...
            : AppPath(KeyUrl("UserLevelPermissionsDelete")); // DN
        ...
        public string GetKey(bool current = false)
        {
            ...
            if (Empty(val))
                return String.Empty;
            keys.Add(val);
            val = current ? ConvertToString(_TableName.CurrentValue) ?? "" : ConvertToString(_TableName.OldValue) ?? "";
            if (Empty(val))
                return String.Empty;
            keys.Add(val);
        ...
        public string GetKey(IDictionary<string, object> row)
        {
            ...
            val = row?.TryGetValue("TableName", out result) ?? false ? ConvertToString(result) : null;
            if (Empty(val))
                return String.Empty; // Invalid key
            keys.Add(ConvertToString(val)); // Add key value
            return String.Join(Config.CompositeKeySeparator, keys);
        ...
        public void SetKey(string key, bool current = false)
        {
            ...
            if (keys.Length == 2) {
                ...
                if (current) {
                    _TableName.CurrentValue = keys[1];
                } else {
                    _TableName.OldValue = keys[1];
                }
            }
        }
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            ...
            _TableName.SetDbValue(dr["TableName"]);
            Permission.SetDbValue(dr["Permission"]);
        }
        ...
        public async Task RenderEditRow()
        {
            ...
            _TableName.EditValue = HtmlEncode(_TableName.CurrentValue);
            _TableName.PlaceHolder = RemoveHtml(_TableName.Caption);

            // Permission
            Permission.SetupEditAttributes();
            Permission.EditValue = Permission.CurrentValue; // DN
        ...
        public async Task WriteAuditTrailLog(string log) => await WriteAuditTrailAsync(CurrentUser(), log, "UserLevelPermissions");
        ...
        public async Task WriteAuditTrailOnAdd(IDictionary<string, object> rs)
        {
            if (!AuditTrailOnAdd)
                return;
            string table = "UserLevelPermissions";
