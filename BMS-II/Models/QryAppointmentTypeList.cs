...
public partial class JUN26 {
    /// <summary>
    /// qryAppointmentTypeList
    /// </summary>
    public static QryAppointmentTypeList qryAppointmentTypeList
    {
        get => HttpData.Get<QryAppointmentTypeList>("qryAppointmentTypeList")!;
        set => HttpData["qryAppointmentTypeList"] = value;
    }

    /// <summary>
    /// Page class for qryAppointmentType
    /// </summary>
    public class QryAppointmentTypeList : QryAppointmentTypeListBase
    {
        // Constructor
        public QryAppointmentTypeList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public QryAppointmentTypeList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class QryAppointmentTypeListBase : QryAppointmentType
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "qryAppointmentType";

        // Page object name
        public string PageObjName = "qryAppointmentTypeList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fqryAppointmentTypelist";
        ...
        // Constructor
        public QryAppointmentTypeListBase()
        {
            // CSS class name as context
            TableVar = "qryAppointmentType";
            ...
            // Table object (qryAppointmentType)
            if (qryAppointmentType == null || qryAppointmentType is QryAppointmentType)
                qryAppointmentType = this;

            // Initialize URLs
            AddUrl = "QryAppointmentTypeAdd";
            ...
            MultiDeleteUrl = "QryAppointmentTypeDelete";
            MultiUpdateUrl = "QryAppointmentTypeUpdate";
        ...
        // Page name
        public string PageName => "QryAppointmentTypeList";
        ...
        // Constructor
        public QryAppointmentTypeListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "QryAppointmentTypeView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fqryAppointmentTypegrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                qryAppointmentTypeList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fqryAppointmentTypesrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fqryAppointmentTypesrch", filters);
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryAppointmentTypelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fqryAppointmentTypelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fqryAppointmentTypelist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fqryAppointmentTypesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fqryAppointmentTypesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fqryAppointmentTypelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_qryAppointmentType");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(qryAppointmentTypeList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(qryAppointmentTypeList.RowCount) + "_qryAppointmentType");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(qryAppointmentTypeList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && qryAppointmentTypeList.RowType == RowType.Add || IsEdit && qryAppointmentTypeList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                int_AppType.ViewValue = FormatNumber(int_AppType.ViewValue, int_AppType.FormatPattern);
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fqryAppointmentTypesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
